 ...
 *
 * The owner hit exactly that on 2026-08-11 in the driver app: a passenger
 * cancelled a trip, the app was open, and "tapping the notification" appeared to
 * do nothing — the app had simply never moved. Both apps carried the same gap.
 *
 ...
 * ⚠️ `otp` is excluded: the code arrives on the OTP screen the user is already
 * looking at, and `routeForNotification` has no destination for it.
 *
...
export function setupForegroundNotificationHandler(
  ...
  const unsubscribe = messaging().onMessage(async (remoteMessage: any) => {
    ...
    if (!onTap || !data || data.type === 'otp') return;
...
/**
 * Handle the user TAPPING a notification (OR-010 item 5).
 *
