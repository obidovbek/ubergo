...
describe('getErrorMessage', () => {
  ...
  it.each([
    ...
    // "Aborted", in English, to an Uzbek user. `handleBackendError` already knew better (T-123).
  ...
  it('a local error that is not a connection failure keeps its own message', () => {
    // The guard against a branch that swallows: the apps throw their own sentences too.
    expect(getErrorMessage(new Error('Rasm tanlanmadi'), t)).toBe('Rasm tanlanmadi');
  });
