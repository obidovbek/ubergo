...
- [x] 8. **DONE — committed by the owner as `6b84aaf` 2026-08-08.**
...
- **2026-08-08 (end of day)** — steps 1-6 finished; 7 and 8 are the owner's. The session closed on a
  **new** problem, not this card's: the owner deployed T-038 and is **still being logged out**. That
  is now **T-041**, and it is the first thing to pick up — see its entry in `docs/TODO.md`, which
  carries the interrupted investigation (one likely-benign explanation and one real defect).
