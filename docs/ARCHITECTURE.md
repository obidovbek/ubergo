...
| Place matching (the four order scopes) | Matches a passenger's route to a driver's places by ids: district for *Viloyatlar aro* / *Viloyat ichi*, the QFY for *Tuman ichi* / *Yaqin* | 🔨 **passenger side built (T-102e 2026-09-13, T-102i 2026-09-19)** — ONE pure rule module (`geoMatch.placeHit`); the SQL is rendered from clauses derived from it, a parity test holds the two together. After an order is created or edited, the search runs **"for your order"** and matches at the order's own level; a driver who named only the district is let through and **labelled**. Offers with no place rows fall back to free text. ⚠️ **SQL-verified, not DB-verified; never on a device.** 🛑 The DRIVER side still matches orders by text → **T-128** | API `src/utils/geoMatch.ts` · `src/utils/offerGeoQuery.ts` · `DriverOfferService.getPublicOffers` · user app `screens/SearchOffersScreen.tsx` · `utils/lastSearch.ts` |
...
| Paynet top-ups | A customer hands cash to a Paynet agent; **Paynet's terminal calls us** | 🔨 **all five methods we offer built and VERIFIED against the real test3 database 2026-08-16 (T-088).** ✅ Proven: a repeated `transactionId` credits **once**; cancel restores the balance; unknown txn → 203; statement lists it once. 🔴 **`PAYNET_CALLS_US` — there is no outbound charge API in this contract.** ✅ **`ChangePassword` is optional and deliberately NOT offered** (answers `603`; owner, 2026-09-19) — the password comes by a secure channel and lives in env, so nothing can rotate us out. ✅ A bad or missing login answers **HTTP 401** (spec §2.2; it answered 200 until 2026-09-19). 🛑 Blocked on **T-100** (IP allow-list) and on Paynet's credentials | `.../services/PaynetService.ts` · `controllers/PaynetController.ts` · `routes/paynet.routes.ts` · `middleware/paynet{Access,Auth}.ts` · `utils/paynet/` · **contract in `docs/PAYNET.md`** |
...
| Tests & CI | Automatic tests for the three code projects, run on every push | ✅ **built 2026-09-14 (T-118)** — `npm test` is ONE command in each. **API:** `node:test` + `tsx`, 391 tests (2026-09-19), DB-free `utils/` and `i18n/` only (the server *flow* tests are still T-010, blocked on a test DB). **Both apps:** `jest && node scripts/run-checks.mjs` — Jest (`jest-expo`, RNTL 13, explicit `@jest/globals`) then all 12 `check-*.mjs` checkers each; **278 + 295 tests** (2026-09-19) — T-118 covered the screens, T-121 (2026-09-18) every `utils/` module, T-116 the error texts (both apps' `getErrorMessage` and `handleBackendError` read ONE rule; `check-raw-error-toasts.mjs` keeps raw `.message` off every toast). A render harness per app (`test/render.tsx` + `test/setup.ts`, **duplicated on purpose** like the shared components) gives real navigation, stubbed auth and the real translations — **a screen that renders a missing translation key FAILS**. 🔴 **No snapshot tests** — T-101 is repainting every screen. Every test file was proven able to go red by mutating the code, then reverted. ⚠️ **The admin panel has none.** ⚠️ Tests cannot see fonts and weights, layout against the artboard, SMS autofill, push delivery, the native build or Google SSO — `docs/CHECKLIST.md` keeps those | `.github/workflows/test.yml` · each app `test/` + `*.test.ts(x)` · `scripts/run-checks.mjs` · API `src/utils/*.test.ts` |
