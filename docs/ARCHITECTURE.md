...
| Tests & CI | Automatic tests for the three code projects, run on every push | ✅ **built 2026-09-14 (T-118)** — `npm test` is ONE command in each. **API:** `node:test` + `tsx`, 357 tests, DB-free `utils/` only (the server *flow* tests are still T-010, blocked on a test DB). **Both apps:** `jest && node scripts/run-checks.mjs` — Jest (`jest-expo`, RNTL 13, explicit `@jest/globals`) then all 11 `check-*.mjs` checkers each; **252 + 278 tests** — T-118 covered the screens, T-121 (2026-09-18) every `utils/` module. A render harness per app (`test/render.tsx` + `test/setup.ts`, **duplicated on purpose** like the shared components) gives real navigation, stubbed auth and the real translations — **a screen that renders a missing translation key FAILS**. 🔴 **No snapshot tests** — T-101 is repainting every screen. Every test file was proven able to go red by mutating the code, then reverted. ⚠️ **The admin panel has none.** ⚠️ Tests cannot see fonts and weights, layout against the artboard, SMS autofill, push delivery, the native build or Google SSO — `docs/CHECKLIST.md` keeps those | `.github/workflows/test.yml` · each app `test/` + `*.test.ts(x)` · `scripts/run-checks.mjs` · API `src/utils/*.test.ts` |
