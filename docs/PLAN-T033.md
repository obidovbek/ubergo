...
✅ **Owner decided 2026-08-08: do NOT change `otpSendLimiter`.** It stays at **5 sends per phone per
hour**. It is now legible (a translated 429) instead of a `JSON Parse error`, and that is enough.
Do not revisit this — a testing session that hits the 6th code in an hour is expected behaviour.
