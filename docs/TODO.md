...
> **The sections.** *(Rewritten by T-122, 2026-09-19. The notes that stood here were dated
> 2026-08-11/12 and had long stopped being true — they are in `docs/TODO-ARCHIVE.md`.)*
> 🔥 **Now** — at most **2** cards: the ones being worked.
> ⏸️ **Parked** — code-complete, waiting only on the owner's phone. Not counted against *Now*.
> 📋 **Next** — ready to start, or waiting on an owner answer (marked 🛑).
> 💡 **Later** — ideas and small finds. · ✅ **Done** — closed, newest on top.
>
> **One card, one copy.** A card that changes section MOVES — it is never copied. A stale copy,
> or a note that stopped being true, goes to `docs/TODO-ARCHIVE.md` verbatim.
> `node scripts/check-board.mjs` checks it (one *Now*, ≤ 2 cards in it, every id carded once,
> no pasted blocks), and `/end-day` runs it. **Where the work stands today lives in
> `docs/PLAN.md`'s Resume point**, not in a dated note here — the one archived from this spot
> said *"No Claude work remains anywhere"* for five weeks after it stopped being true.
...
- [ ] T-125 (P3) 🧹 **WHAT DOES *DONE* MEAN? At least four cards sit in *Done* that were never
  ...
  *Parked*'s own note says *Done* means **owner device-confirmed** (*"that is how T-041, T-042 and
  T-044 got there"*). But **T-024** is in *Done* while `PLAN-T024.md` step 7 — *"Owner: rebuild
  the user app, walk passenger→driver→accept end to end"* — is still unchecked, and the 2026-08-11
  header (now in `docs/TODO-ARCHIVE.md`) listed **T-034 · T-043 · T-045** as code-complete and
  untested; all three are in *Done* too.
  ❓ **OWNER DECISION:** does *Done* mean *committed* or *confirmed on a phone*? If the second, the
  unwalked cards move to *Parked*; if the first, *Parked*'s note is rewritten. *Recommendation:
  confirmed on a phone* — it is the only meaning that tells the owner what is left to walk.
  ⚠️ **Measure every *Done* card against its plan file's device step before moving any** — four
  is what was noticed, not what was counted. ❌ No code.
...
- [x] T-122 (P2) 🧹 **THE BOARD CONTRADICTED ITSELF — TWO *Now* SECTIONS, 22 DUPLICATED CARDS —
  ...
  *"any, you choose"*), the plan approved the same day with **T-101** as the other *Now* card.
  ...
  (T-078 four times), and ***Now* held 23 cards** — most *"code-complete and untested"*, which is
  word for word the definition of *Parked*, one section down.
  ✅ **One *Now*, every id carded once.** 14 cards → *Parked*, 5 → *Next*, each by its own status
  line. **20 stale copies, 7 notes and T-097** (this same card, boarded 2026-08-15 and never done)
  → **`docs/TODO-ARCHIVE.md`, verbatim. 352 KB → 273 KB.**
  ✅ **Nothing lost, and proven:** every card and every line of `fed25f9`'s board survives in
  board ∪ archive — a check that went red on each mutation aimed at it.
  ✅ **`scripts/check-board.mjs` keeps it this way**, and `/end-day` now runs it.
  📌 **Found, not fixed → T-125:** *Done* holds cards never walked on a device.
