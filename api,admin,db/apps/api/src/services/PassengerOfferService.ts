...
type PassengerOfferWritableFields = Partial<
  Omit<PassengerOfferAttributes, 'id' | 'user_id' | 'status' | 'created_at' | 'updated_at'>
>;
...
export class PassengerOfferService {
   ...
   */
  private static parseText(value: unknown, field: string, maxLength?: number): string | null {
   ...
   */
  private static parseSeatCounts(value: unknown): PassengerOfferSeatCounts | null {
    if (value === null || value === undefined) return null;
    ...
    for (const key of this.SEAT_COUNT_KEYS) {
      ...
      if (!Number.isInteger(count) || count < 0) {
        throw new AppError(`seat_counts.${key} must be a non-negative integer`, 400);
      }
      counts[key] = count;
    }

    if (counts.front_male + counts.front_female > this.MAX_FRONT_SEATS) {
      throw new AppError(`seat_counts front seats must not exceed ${this.MAX_FRONT_SEATS}`, 400);
    }
    if (counts.back_male + counts.back_female > this.MAX_BACK_SEATS) {
      throw new AppError(`seat_counts back seats must not exceed ${this.MAX_BACK_SEATS}`, 400);
    }
  ...
  private static sumSeatCounts(counts: PassengerOfferSeatCounts): number {
    return counts.front_male + counts.front_female + counts.back_male + counts.back_female;
  }

  private static parseVehicleTypes(value: unknown): PassengerOfferVehicleType[] | null {
    if (value === null || value === undefined) return null;
   ...
   */
  private static parseSpecialOrder(value: unknown): PassengerOfferSpecialOrder | null {
    if (value === null || value === undefined) return null;
    if (typeof value !== 'object' || Array.isArray(value)) {
    ...
    const hasPrice = this.SPECIAL_ORDER_PRICE_KEYS.some((key) => special[key] != null);
    if (!hasPrice) {
  ...
  private static buildOfferFields(
    ...
    for (const key of ['depart_until', 'arrive_from', 'arrive_until'] as const) {
      if (sent(key)) fields[key] = this.parseDate(body[key], key);
    }

    if (sent('is_urgent')) fields.is_urgent = Boolean(body.is_urgent);
    const isUrgent = (sent('is_urgent') ? fields.is_urgent : current?.is_urgent) ?? false;
    // ⚡ "hozioq" — leaving right now, so there is no departure window
    if (isUrgent) fields.depart_until = null;

    // ── Payment ─────────────────────────────────────────────────────────────
    if (sent('max_price_per_seat')) {
      fields.max_price_per_seat = this.parseNumber(body.max_price_per_seat, 'max_price_per_seat');
    }
    if (sent('currency')) {
    ...
    if (sent('payment_type')) {
      fields.payment_type = this.parseEnum(body.payment_type, this.PAYMENT_TYPES, 'payment_type');
    }
    if (sent('payer_phone')) {
    ...
    // ── Seats ───────────────────────────────────────────────────────────────
    if (sent('seat_position_any')) fields.seat_position_any = Boolean(body.seat_position_any);
    if (sent('salon_scope')) {
      fields.salon_scope = this.parseEnum(body.salon_scope, this.SALON_SCOPES, 'salon_scope');
    }
    if (sent('seat_counts')) fields.seat_counts = this.parseSeatCounts(body.seat_counts);

    const salonScope = (sent('salon_scope') ? fields.salon_scope : current?.salon_scope) ?? null;
    let seatCounts = (sent('seat_counts') ? fields.seat_counts : current?.seat_counts) ?? null;
    ...
    if (!isUpdate || sent('seats_needed') || sent('seat_counts') || sent('salon_scope')) {
      ...
      if (seatsNeeded === null) {
        throw new AppError('seats_needed, seat_counts or salon_scope is required', 400);
      }
      if (
    ...
    if (sent('vehicle_types')) fields.vehicle_types = this.parseVehicleTypes(body.vehicle_types);
    ...
    if (sent('road_pickup_note')) {
      fields.road_pickup_note = this.parseText(body.road_pickup_note, 'road_pickup_note');
    }
    if (sent('note')) fields.note = this.parseText(body.note, 'note');
    if (sent('special_order')) fields.special_order = this.parseSpecialOrder(body.special_order);
  ...
  private static validateOfferData(
    ...
    const merged = {
      ...
      is_urgent: (has('is_urgent') ? fields.is_urgent : current?.is_urgent) ?? false,
      depart_until: (has('depart_until') ? fields.depart_until : current?.depart_until) ?? null,
      arrive_from: (has('arrive_from') ? fields.arrive_from : current?.arrive_from) ?? null,
      arrive_until: (has('arrive_until') ? fields.arrive_until : current?.arrive_until) ?? null,
      currency: (has('currency') ? fields.currency : current?.currency) ?? 'UZS',
      payment_type: (has('payment_type') ? fields.payment_type : current?.payment_type) ?? null,
      payer_phone: (has('payer_phone') ? fields.payer_phone : current?.payer_phone) ?? null
    };
    ...
    if (merged.start_at && merged.depart_until && merged.depart_until < merged.start_at) {
      throw new AppError('depart_until must not be before start_at', 400);
    }
    if (merged.start_at && merged.arrive_from && merged.arrive_from < merged.start_at) {
      throw new AppError('arrive_from must not be before start_at', 400);
    }
    if (merged.arrive_from && merged.arrive_until && merged.arrive_until < merged.arrive_from) {
    ...
    if (merged.payment_type === 'friend_pays' && !merged.payer_phone) {
      throw new AppError('payer_phone is required when payment_type is friend_pays', 400);
    }
  }
