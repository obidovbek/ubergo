 ...
 * The first run failed halfway (see below) and left `wallet_accounts` behind
 * with no `wallet_transactions` and no SequelizeMeta row — so re-running then
 * failed again with "already exists". Postgres has transactional DDL; using it
 * means a failure leaves the database exactly as it was.
 *
 * 🔴 `actor_admin_id` IS A UUID, NOT AN INTEGER.
 * This schema has TWO different id types: `users.id` is INTEGER and
 * `admin_users.id` is UUID. The first version of this migration assumed admins
 * were integers like users and Postgres rejected the foreign key outright
 * ("Key columns are of incompatible types: integer and uuid"). Both actor
 * columns are deliberately kept — an admin and a user are different subjects —
 * so they simply have different types.
 *
...
module.exports = {
    ...
    await queryInterface.sequelize.transaction(async (transaction) => {
      await queryInterface.createTable(
        'wallet_accounts',
        {
          id: {
            type: Sequelize.UUID,
            // uuid_generate_v4(), not gen_random_uuid(): the "uuid-ossp"
            // extension is created by 20250118000001 and every UUID pk in this
            // schema uses it.
          ...
          // entries are the truth; the reconciliation check proves they agree.
          balance: {
      ...
      await queryInterface.createTable(
        'wallet_transactions',
        {
          // BIGSERIAL, not UUID, ON PURPOSE: Paynet's `providerTrnId` is the id
          // WE mint and they quote back at us, and their samples show a NUMBER
          // (`"providerTrnId": 2323`). Reusing the primary key means T-088 needs
          // no second identity column and no second thing to keep unique.
          id: {
            type: Sequelize.BIGINT,
            primaryKey: true,
            autoIncrement: true,
          ...
          // SIGNED: negative is a debit, so the sum of the entries IS the
          // balance and reconciliation is one GROUP BY. The alternative — a
          // `direction` column with positive amounts — makes every balance query
          // a CASE expression and gives two ways to write the same fact.
          amount: {
            type: Sequelize.BIGINT,
            allowNull: false
          },
          // The account balance immediately after this entry. Makes every row
          // self-verifying; only correct because writes are serialised by the
          // row lock the service takes.
          balance_after: {
            type: Sequelize.BIGINT,
            allowNull: false
          },
          ...
          // 🔴 UUID: admin_users.id is a UUID while users.id is an INTEGER.
          actor_admin_id: {
            type: Sequelize.UUID,
            allowNull: true,
          ...
          // The PROVIDER'S id for this payment (Paynet's `transactionId`). This
          // is the idempotency key — see the partial unique index below.
          external_id: {
      ...
      // 🔴 AND THIS CHECK IS WHAT MAKES THE INDEX BELOW ACTUALLY UNIQUE.
      // In Postgres NULL is never equal to NULL, so in a MULTI-COLUMN unique
      // index any row with a NULL in an indexed column conflicts with nothing:
      // (NULL, 'TRX-1') and (NULL, 'TRX-1') would BOTH insert, and the
      // idempotency guarantee would silently not hold for an entry whose
      // provider was not set. Requiring a provider whenever there is an
      // external id closes that hole. Found by rendering the generated SQL
      // before the migration was ever run.
      await queryInterface.sequelize.query(
        'ALTER TABLE wallet_transactions ADD CONSTRAINT chk_wallet_transactions_external_needs_provider ' +
      ...
      // существует") is the mandated answer to a repeat; this index is what
      // makes that answer possible. It CANNOT be added safely after the fact.
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      await queryInterface.dropTable('wallet_accounts', { transaction });
    });
  }
};
