/**
 * T-121 step 2 — `tokenStore.ts`, the single owner of the access/refresh token pair (T-038).
 *
 ...
 *
 * ⚠️ The user app's copy of `tokenStore.ts` is byte-identical and has its own copy of this
 * file (step 1). Change both. Compare the two with `git show HEAD:<path>` rather than a
 * working-tree `diff` — `core.autocrlf` is on, so a `git checkout` rewrites line endings and
 * makes two identical files look entirely different.
 */
