/**
 * Russian Translations
 */

export default {
  // T-054 — shared by utils/contactPhone.ts, which any screen showing a
  // confirmed counterparty's number can use.
  contact: {
    noPhone: 'Номер телефона не указан',
    dialFailed: 'Не удалось начать звонок',
    noEmail: 'Адрес электронной почты не указан',
    emailFailed: 'Не удалось открыть почтовое приложение',
  },
  common: {
    continue: 'Продолжить',
    cancel: 'Отмена',
    delete: 'Удалить',
    confirm: 'Подтвердить',
    ok: 'ОК',
    error: 'Ошибка',
    success: 'Успешно',
    loading: 'Загрузка...',
    retry: 'Повторить',
    back: 'Назад',
    next: 'Далее',
    saving: 'Сохранение...',
    datePlaceholder: 'ДД.ММ.ГГГГ',
    selectDate: 'Выберите дату',
    day: 'День',
    month: 'Месяц',
    year: 'Год',
    // Used by the shared modal components (T-036) — kept under `common` so the copies
    // in both apps stay identical and independent of any screen's namespace.
    search: 'Поиск...',
    noResults: 'Ничего не найдено',
    hour: 'Час',
    minute: 'Минута',
    camera: 'Камера',
    gallery: 'Галерея',
    // T-057 — the PhotoSourceModal's fallback heading. Screens with their own
    // wording (driverLicense.selectPhoto, …) pass it explicitly.
    selectPhoto: 'Выбрать фото',
    // T-057 — three screens had these hard-coded in Uzbek, so a Russian or
    // English user was shown Uzbek. The two older screens keep their own
    // screen-scoped copies, which were already translated.
    cameraPermissionMessage: 'Разрешите доступ к камере',
    galleryPermissionMessage: 'Разрешите доступ к галерее',
    permissionRequired: 'Требуется разрешение',
    // `all`/`viewAll` existed in uz only — the i18n check caught it when the
    // T-037 filter bar asked for `common.all` (T-037 step 5).
    viewAll: 'Посмотреть все',
    all: 'Все',
    monthJanuary: 'Январь',
    monthFebruary: 'Февраль',
    monthMarch: 'Март',
    monthApril: 'Апрель',
    monthMay: 'Май',
    monthJune: 'Июнь',
    monthJuly: 'Июль',
    monthAugust: 'Август',
    monthSeptember: 'Сентябрь',
    monthOctober: 'Октябрь',
    monthNovember: 'Ноябрь',
    monthDecember: 'Декабрь',
    info: 'Информация',
  },


  splash: {
    appName: 'UbexGo',
    tagline: 'Водите и зарабатывайте',
    loading: 'Загрузка...',
  },

  auth: {
    appName: 'UbexGo',
    // T-061 — see the uz file: a brand word, deliberately untranslated.
    appNameDriverSuffix: 'Driver',
    logout: 'Выйти',
    accountBlocked: 'Аккаунт заблокирован',
    accountBlockedDescription: 'К сожалению, ваш аккаунт заблокирован. Пожалуйста, свяжитесь со службой поддержки или попробуйте позже.',
    accountPendingDelete: 'Удаление аккаунта',
    accountPendingDeleteDescription: 'Ваш аккаунт находится в процессе удаления. Если это ошибка, пожалуйста, свяжитесь со службой поддержки.',
    accountSuspended: 'Аккаунт приостановлен',
    accountSuspendedDescription: 'Ваш аккаунт временно приостановлен. Пожалуйста, свяжитесь со службой поддержки для получения помощи.',
    contactSupport: 'Связаться с поддержкой',
    supportEmail: 'Email: support@ubexgo.uz',
    supportPhone: 'Телефон: +998 XX XXX XX XX',
    emailNotAvailable: 'Почтовый клиент недоступен на этом устройстве',
    emailError: 'Не удалось открыть почтовый клиент',
    phoneNotAvailable: 'Телефон недоступен на этом устройстве',
    phoneError: 'Не удалось открыть телефон',
  },

  searchPassengerOffers: {
    from: 'Откуда',
    to: 'Куда',
    filter: 'Фильтры',
    applyFilters: 'Применить фильтры',
    loginRequired: 'Войдите, чтобы посмотреть заказ',
    // T-038/T-037 step 2: this screen was almost entirely hard-coded English —
    // it had never been reachable, so nobody had seen it.
    title: 'Заказы пассажиров',
    fromLabel: 'ОТКУДА',
    toLabel: 'КУДА',
    clear: 'Очистить',
    maxBudget: 'Макс. цена',
    perSeat: 'за место',
    viewDetails: 'Подробнее',
    loading: 'Загрузка заказов...',
    emptySelectTitle: 'Выберите маршрут',
    emptySelectSubtitle: 'Укажите откуда и куда, чтобы увидеть заказы пассажиров',
    emptyNoneTitle: 'Заказов нет',
    emptyNoneSubtitle: 'На этом маршруте сейчас нет заказов пассажиров',
    resultsCount: 'Заказов: {count}',
    sortBy: 'Сортировка',
    sortDateAsc: 'Дата (сначала ранние)',
    sortPriceDesc: 'Цена (сначала высокие)',
    sortPriceAsc: 'Цена (сначала низкие)',
    sortSeatsDesc: 'Места (сначала больше)',
    filtersTitle: 'Фильтры',
    maxBudgetLabel: 'Максимальная цена (за место)',
    maxBudgetPlaceholder: 'Любая',
    minSeatsLabel: 'Минимум мест',
    minSeatsPlaceholder: 'Любое',
    clearAllFilters: 'Сбросить все фильтры',
    // T-049: the offer card used to render hard-coded English ("2 seats needed").
    seatsNeededCount: 'Нужно мест: {count}',
    // T-049: eight more hard-coded English toasts lived in this screen's geo
    // pickers. They only fire on an error or an out-of-order tap, which is why
    // nobody had seen them.
    loadProvincesFailed: 'Не удалось загрузить области',
    loadCitiesFailed: 'Не удалось загрузить города',
    selectCountryFirst: 'Сначала выберите страну',
    selectProvinceFirst: 'Сначала выберите область',
  },

  // Passenger order details (driver side) — T-037
  passengerOfferDetails: {
    title: 'Детали заказа',
    from: 'ОТКУДА',
    to: 'КУДА',
    departure: 'Отправление',
    passenger: 'Пассажир',
    passengerUnknown: 'Не указан',
    seatsNeeded: 'Нужно мест',
    maxPrice: 'Максимальная цена',
    priceNotSet: 'Не указана',
    note: 'Примечание',
    notFound: 'Заказ не найден',
    loadFailed: 'Ошибка загрузки заказа',
    takeOrder: 'Взять этот заказ',
    joinTitle: 'Отправьте предложение',
    vehicle: 'Автомобиль',
    noVehicle: 'У вас не добавлен автомобиль. Сначала добавьте автомобиль в профиле.',
    seatsOffered: 'Предлагаемые места',
    seatsHint: 'Пассажиру нужно минимум {count} мест',
    pricePerSeat: 'Цена за место',
    total: 'Итого',
    messageLabel: 'Сообщение (необязательно)',
    messagePlaceholder: 'Сообщение пассажиру...',
    send: 'Отправить',
    sending: 'Отправка...',
    sentTitle: 'Предложение отправлено',
    sentMessage: 'Дождитесь ответа пассажира',
    alreadySent: 'Ваше предложение отправлено',
    statusConfirmed: 'Ваше предложение подтверждено',
    statusRejected: 'Ваше предложение отклонено',
    statusCancelled: 'Вы отменили своё предложение',
    joinFailed: 'Ошибка отправки предложения',
    errorPrice: 'Введите корректную цену',
    errorSeats: 'Недостаточно мест',
  },

  // Offers the driver has sent on passenger orders — T-037
  myJoinRequests: {
    title: 'Мои предложения',
    empty: 'Вы ещё не отправляли предложений',
    loadFailed: 'Ошибка загрузки предложений',
    offerGone: 'Заказ недоступен',
    pricePerSeat: 'Цена за место',
    seatsOffered: 'Предложено мест',
    total: 'Итого',
    status_pending: 'Ожидает',
    status_confirmed: 'Подтверждено',
    status_rejected: 'Отклонено',
    status_cancelled: 'Отменено',
    cancel: 'Отменить предложение',
    cancelTitle: 'Отменить предложение?',
    cancelWarning:
      'После отмены вы не сможете снова предложить себя на этот заказ. Это необратимо.',
    cancelConfirm: 'Да, отменить',
    cancelled: 'Предложение отменено',
    cancelFailed: 'Ошибка отмены',
    contactTitle: 'Связь с пассажиром',
    noPhone: 'У пассажира не указан номер телефона',
  },

  phoneRegistration: {
    brand: 'UbexGo Driver',
    selectCountry: 'Выберите страну',
    title: 'Регистрация водителя',
    driverType: 'Водитель',
    instruction: 'Введите номер телефона, зарегистрированный в приложении UbexGo',
    orIdText: 'или ваш ID номер UbexGo',
    termsText: 'Нажимая "Продолжить", я принимаю условия ',
    termsLink: 'пользовательского соглашения',
    termsText2: '',
    phonePlaceholder: '00 000-00-00',
    googleSignIn: 'Войти через Google',
    appleSignIn: 'Продолжить с Apple',
    facebookSignIn: 'Войти через Facebook',
    microsoftSignIn: 'Войти через Microsoft',
    signingIn: 'Вход...',
    footerText: 'Регистрируясь через указанные платформы, я принимаю ',
    footerLink: 'условия использования',
    footerText2: '',
    alreadyHaveAccount: 'Уже есть аккаунт? ',
    login: 'Войти',
    errorPhoneIncomplete: 'Пожалуйста, введите полный номер телефона (9 цифр)',
    errorGoogleNotReady: 'Сервис Google не готов. Пожалуйста, попробуйте снова.',
    errorGoogleCancelled: 'Вход через Google отменен',
    errorNetwork: 'Проблема с интернет-соединением. Пожалуйста, попробуйте снова.',
    errorGoogleFailed: 'Ошибка входа через Google',
    infoNotAvailable: ' вход пока недоступен',
  },

  // Register First Screen
  registerFirst: {
    title: 'Сначала зарегистрируйтесь в приложении пользователя',
    description: 'Чтобы зарегистрироваться как водитель, вы должны сначала зарегистрироваться в приложении UbexGo для пользователей с вашим номером телефона.',
    step1: 'Скачайте приложение UbexGo для пользователей',
    step2: 'Зарегистрируйтесь в приложении с вашим номером телефона',
    step3: 'Затем вернитесь в приложение водителя и продолжите с тем же номером телефона',
    downloadAndroid: 'Скачать для Android',
    downloadIOS: 'Скачать для iOS',
    noUrlsAvailable: 'Ссылки на магазины приложений пока не настроены',
  },

  otpVerification: {
    brand: 'UbexGo Driver',
    title: 'Введите код',
    subtitle: 'Введите код подтверждения, полученный в вашем аккаунте приложения UbexGo',
    verifying: 'Проверка...',
    resendQuestion: 'Не получили код? ',
    resendLink: 'Отправить снова',
    resendIn: 'Отправить снова ({seconds} с)',
    newCodeSent: 'Новый код отправлен через push-уведомление',
    errorResendPush:
      'Ошибка отправки push-уведомления. Убедитесь, что приложение пассажира открыто.',
    errorIncomplete: 'Пожалуйста, введите полный код подтверждения',
    errorIncorrect: 'Неверный код. Осталось попыток: ',
    errorNoAttempts: 'Ошибка',
    errorNoAttemptsMessage: 'Попытки исчерпаны. Пожалуйста, попробуйте снова',
    errorResend: 'Ошибка отправки кода',
    phoneVerified: 'Телефон подтвержден',
    attemptsLabel: 'Попытка: ',
    remainingLabel: ' | Осталось: ',
  },

  userDetails: {
    title: 'Междугородние',
    subtitle: 'Заполните форму, чтобы получить\nбонусы и подарки',
    requiredField: '* - обязательное поле',
    firstName: 'ИМЯ',
    lastName: 'Фамилия',
    fatherName: 'Отчество',
    gender: 'Пол',
    male: 'Мужской',
    female: 'Женский',
    birthDate: 'Дата рождения',
    email: 'email',
    phone: 'Тел.:',
    userId: 'ID:',
    promoCode: 'ПРОМО:',
    additionalPhones: 'Введите дополнительные номера\nдля связи\n(до 5. На случай потери сигнала....)',
    submit: 'Готово',
    firstNamePlaceholder: 'Введите ваше имя',
    lastNamePlaceholder: 'Введите вашу фамилию',
    fatherNamePlaceholder: 'Отчество',
    emailPlaceholder: 'example@email.com',
    userIdPlaceholder: '1 001 117',
    promoCodePlaceholder: '1515',
    birthDatePlaceholder: '21.01.1985',
    phonePlaceholder: '90 123 45 67',
    genderHelper: 'Чтобы найти подходящий транспорт',
    birthDateHelper: 'Для подарка от компании в день рождения',
    emailHelper: 'Для восстановления аккаунта',
    infoBoxText: 'Человек, который пригласил вас (отправил ссылку) ',
    infoBoxPhone: 'номер телефона',
    infoBoxId: '\nID номер',
    infoBoxPromo: '\nили промо ',
    infoBoxPromoCode: 'ПРОМО код',
    infoBoxPromo2: ' чтобы получить\n',
    infoBoxBonus: '10 000',
    infoBoxBonus2: ' бонус',
    selectDate: 'Выберите дату рождения',
    day: 'День',
    month: 'Месяц',
    year: 'Год',
    errorFirstName: 'Имя обязательно',
    errorLastName: 'Фамилия обязательна',
    errorGender: 'Выберите пол',
    errorBirthDate: 'Введите дату рождения',
    errorBirthDateIncomplete: 'Введите полную дату',
    errorEmail: 'Email обязателен',
    errorEmailInvalid: 'Неверный формат email',
    errorPhone: 'Пожалуйста, введите полный номер телефона',
    errorAllFields: 'Пожалуйста, заполните все обязательные поля',
    successTitle: 'Успешно!',
    successMessage: 'Регистрация завершена успешно',
    errorUpdate: 'Ошибка обновления профиля',
  },

  menu: {
    /** T-101: the top bar screen name. `title` is the CARD heading. */
    screenTitle: 'Междугородние',
    title: 'ТАКСИ',
    subtitle: 'Выберите страну',
    uzbekistan: 'Узбекистан',
    guest: 'Гость',
    // `myOffers` was rendered from uz only — ru/en showed the raw key on the
    // home screen. Found and fixed during T-037 step 1.
    myOffers: 'Мои объявления',
    passengerOrders: 'Заказы пассажиров',
    myJoinRequests: 'Мои предложения',
    viloyatlar: 'Такси\nМеждугородние',
    ichi: 'Такси\nВ пределах\nгорода',
    tuman: 'Такси\nРайон,\nместные и ближние\nрасстояния',
    empty: 'Такси',
    xalqaro: 'Такси\nМЕЖДУНАРОДНОЕ',
  },

  profile: {
    logout: 'Выйти',
    logoutConfirm: 'Вы уверены, что хотите выйти?',
    logoutError: 'Ошибка выхода. Пожалуйста, попробуйте снова.',
    noEmail: 'Нет email',
    noPhone: 'Нет телефона',
    driverId: 'ID водителя',
    appVersion: 'UbexGo v1.0.0',
    notifications: 'Уведомления',
    editProfile: 'Редактировать профиль',
    paymentMethods: 'Способы оплаты',
    tripHistory: 'История поездок',
    helpSupport: 'Помощь и поддержка',
    settings: 'Настройки',
    title: 'Профиль',
    myOffers: 'Мои объявления',
  },

  months: {
    january: 'Январь',
    february: 'Февраль',
    march: 'Март',
    april: 'Апрель',
    may: 'Май',
    june: 'Июнь',
    july: 'Июль',
    august: 'Август',
    september: 'Сентябрь',
    october: 'Октябрь',
    november: 'Ноябрь',
    december: 'Декабрь',
  },

  // Driver Messages
  driver: {
    profileUpdated: 'Профиль обновлен',
    profileIncomplete: 'Профиль не заполнен',
    registrationComplete: 'Регистрация завершена',
    passportUpdated: 'Данные паспорта сохранены',
    licenseUpdated: 'Данные прав сохранены',
    vehicleUpdated: 'Данные автомобиля сохранены',
    taxiLicenseUpdated: 'Лицензия такси сохранена',
  },

  // Backend Error Messages
  errors: {
    network: 'Нет подключения к интернету',
    timeout: 'Время ожидания истекло',
    serverError: 'Произошла ошибка сервера',
    unauthorized: 'Неавторизован',
    forbidden: 'Доступ запрещен',
    notFound: 'Не найдено',
    badRequest: 'Неверный запрос',
    conflict: 'Данные уже существуют',
    validation: 'Неверные данные',
    unknown: 'Произошла неизвестная ошибка',
    tryAgain: 'Пожалуйста, попробуйте снова',
    tooManyRequests: 'Слишком много запросов. Пожалуйста, попробуйте позже',
    // T-035 — recovered from the overridden block; 9 call sites across both apps.
    loadFailed: 'Не удалось загрузить данные',
    saveFailed: 'Не удалось сохранить данные',
    deleteFailed: 'Не удалось удалить',
    updateFailed: 'Не удалось обновить',
    createFailed: 'Не удалось создать',
  },

  // Network Status
  networkStatus: {
    noInternet: 'Нет подключения к интернету',
    noInternetMessage: 'Мы не можем подключиться к серверу. Пожалуйста, проверьте подключение к интернету, чтобы продолжить получать заказы на поездки.',
    hint: 'Убедитесь, что Wi-Fi или мобильные данные включены',
    retry: 'Попробовать снова',
    tip1: 'Проверьте настройки Wi-Fi или мобильных данных',
    tip2: 'Потяните вниз для обновления после подключения',
  },

  // Form Validation Messages
  formValidation: {
    // Driver Details
    driverTypeRequired: 'Выберите тип водителя',
    driverTypeInvalid: 'Выбран неверный тип водителя',

    // Personal Info
    firstNameRequired: 'Имя обязательно',
    firstNameTooShort: 'Имя слишком короткое',
    lastNameRequired: 'Фамилия обязательна',
    lastNameTooShort: 'Фамилия слишком короткая',
    fatherNameRequired: 'Отчество обязательно',
    genderRequired: 'Выберите пол',
    genderInvalid: 'Выбран неверный пол',
    birthDateRequired: 'Дата рождения обязательна',
    birthDateInvalid: 'Неверный формат даты',
    emailInvalid: 'Неверный формат email',
    addressCountryRequired: 'Страна обязательна',
    addressRegionRequired: 'Регион обязателен',
    addressCityRequired: 'Город обязателен',
    photoRequired: 'Загрузка фото обязательна',

    // Passport Info
    idCardRequired: 'Номер ID карты обязателен',
    idCardTooShort: 'Номер ID карты слишком короткий',
    pinflRequired: 'ПИНФЛ обязателен',
    pinflInvalid: 'ПИНФЛ должен содержать 14 цифр',
    citizenshipRequired: 'Гражданство обязательно',
    issueDateRequired: 'Дата выдачи обязательна',
    expiryDateRequired: 'Срок действия обязателен',

    // License Info
    licenseNumberRequired: 'Номер прав обязателен',
    licenseNumberTooShort: 'Номер прав слишком короткий',
    issueDateInvalid: 'Неверная дата выдачи',
    categoryRequired: 'Необходимо выбрать хотя бы одну категорию',

    // Emergency Contacts
    phoneRequired: 'Номер телефона обязателен',
    phoneInvalid: 'Неверный формат телефона',
    relationshipRequired: 'Родство обязательно',

    // Vehicle Info
    vehicleTypeRequired: 'Тип транспорта обязателен',
    vehicleTypeInvalid: 'Неверный тип транспорта',
    vehicleMakeRequired: 'Выберите марку',
    vehicleModelRequired: 'Выберите модель',
    vehicleBodyTypeRequired: 'Выберите тип кузова',
    vehicleColorRequired: 'Выберите цвет',
    licensePlateRequired: 'Гос номер обязателен',
    licensePlateTooShort: 'Гос номер должен содержать не менее 3 символов',
    techPassportSeriesRequired: 'Серия техпаспорта обязательна',
    techPassportSeriesTooShort: 'Серия техпаспорта должна содержать не менее 3 символов',
    makeRequired: 'Марка обязательна',
    modelRequired: 'Модель/Тип обязателен',
    yearRequired: 'Год выпуска обязателен',
    yearInvalid: 'Неверный формат года',
    yearOutOfRange: 'Год должен быть между 1900 и {maxYear}',
    colorRequired: 'Цвет обязателен',
    ownerFirstNameRequired: 'Имя владельца обязательно',
    ownerFirstNameTooShort: 'Имя владельца должно содержать не менее 2 символов',
    ownerLastNameRequired: 'Фамилия владельца обязательна',
    ownerLastNameTooShort: 'Фамилия владельца должна содержать не менее 2 символов',
    ownerFatherNameRequired: 'Отчество владельца обязательно',
    ownerFatherNameTooShort: 'Отчество владельца должно содержать не менее 2 символов',
    ownerPinflRequired: 'ПИНФЛ владельца обязателен',
    ownerPinflInvalid: 'ПИНФЛ должен содержать 14 цифр',
    ownerAddressCountryRequired: 'Выберите страну',
    ownerAddressProvinceRequired: 'Выберите область',
    ownerAddressCityDistrictRequired: 'Выберите город или район',
    ownerAddressStreetRequired: 'Адрес улицы обязателен',
    ownerAddressStreetTooShort: 'Адрес улицы должен содержать не менее 5 символов',
    fuelTypesRequired: 'Необходимо выбрать хотя бы один тип топлива',
    grossWeightInvalid: 'Полная масса должна быть положительным числом',
    grossWeightTooLarge: 'Полная масса не должна превышать 50000 кг',
    unladenWeightInvalid: 'Снаряженная масса должна быть положительным числом',
    unladenWeightTooLarge: 'Снаряженная масса не должна превышать 50000 кг',
    seatingCapacityInvalid: 'Количество мест должно быть положительным числом',
    seatingCapacityTooLarge: 'Количество мест не должно превышать 50',
    companyNameTooShort: 'Название компании должно содержать не менее 2 символов',

    // Taxi License
    taxiLicenseRequired: 'Номер лицензии такси обязателен',
    taxiLicenseTooShort: 'Номер лицензии должен содержать не менее 3 символов',
    licenseIssueDateRequired: 'Дата выдачи обязательна',
    licenseIssueDateInvalid: 'Неверный формат даты',
    licenseRegistryNumberRequired: 'Реестровый номер лицензии обязателен',
    licenseRegistryNumberTooShort: 'Реестровый номер должен содержать не менее 3 символов',
    licenseSheetRequired: 'Номер лицензии обязателен',
    licenseSheetTooShort: 'Номер лицензии должен содержать не менее 3 символов',
    validFromRequired: 'Дата начала действия обязательна',
    validFromInvalid: 'Неверный формат даты начала действия',
    validUntilRequired: 'Дата окончания действия обязательна',
    validUntilInvalid: 'Неверный формат даты окончания действия',
    validUntilBeforeValidFrom: 'Дата окончания действия должна быть после даты начала действия',
    selfEmploymentNumberRequired: 'Номер самозанятого обязателен',
    selfEmploymentNumberTooShort: 'Номер самозанятого должен содержать не менее 3 символов',

    // Generic messages
    required: 'Это поле обязательно',
    tooShort: 'Значение слишком короткое',
    tooLong: 'Значение слишком длинное',
    invalid: 'Неверный формат',
    mustBeNumber: 'Должно быть числом',
    mustBePositive: 'Должно быть положительным значением',
    fixErrors: 'Пожалуйста, исправьте выделенные ошибки',
    dateInvalid: 'Неверный формат даты',
    dateCannotBeFuture: 'Дата не может быть позже сегодняшней',
    dateCannotBePast: 'Дата не может быть раньше сегодняшней',
  },

  // Driver License Screen
  driverLicense: {
    selectCountryCode: "Выберите код страны",
    title: 'Междугородние',
    subtitle: 'ВОДИТЕЛЬСКИЕ ПРАВА',
    description: '(Вся информация вводится согласно паспорту)',
    issueDate: 'Дата выдачи',
    licenseNumber: 'Номер прав',
    licenseNumberPlaceholder: 'AG 1234567',
    categoryDatePlaceholder: 'Дата выдачи разрешения',
    licensePhoto: 'Водительские права:',
    licensePhotoFrontHint: '(Лицевая сторона с фото)',
    licensePhotoBackHint: '(Обратная сторона)',
    emergencyContactsTitle: 'Введите 2 номера телефонов близких родственников для экстренных случаев',
    emergencyContactsHint: '(отец, мать, брат...)',
    phoneNumberPlaceholder: '0 00 00 00 00',
    relationshipFather: 'Отец',
    relationshipSpouse: 'Супруг(а)',
    selectPhoto: 'Выбрать фото',
    howToSelectPhoto: 'Как вы хотите выбрать фото?',
    cameraPermissionMessage: 'Пожалуйста, предоставьте разрешение на использование камеры',
    galleryPermissionMessage: 'Пожалуйста, предоставьте разрешение на доступ к галерее',
    photoUploaded: 'Фото успешно загружено',
    photoUploadError: 'Ошибка загрузки фото',
    photoReadError: 'Ошибка чтения фото',
    photoSelectError: 'Ошибка выбора фото',
    licenseSaved: 'Данные прав сохранены',
    errorUpdate: 'Ошибка сохранения данных',
    licenseNumberRequired: 'Пожалуйста, введите номер прав',
    issueDateRequired: 'Пожалуйста, введите дату выдачи',
  },

  // Driver Passport Screen — see the uz file for why only these three exist.
  driverPassport: {
    birthPlaceCountry: 'Страна',
    birthPlaceProvince: 'Область',
    birthPlaceCityDistrict: 'Город / Район',
  },

  // Driver Vehicle Screen
  driverVehicle: {
    title: 'ДАННЫЕ ТРАНСПОРТНОГО СРЕДСТВА',
    subtitle: 'РЕДАКТИРОВАНИЕ ДАННЫХ',
    description: '(Все данные вводятся согласно техпаспорту)',
    backButton: 'Назад',
    // Vehicle Basic Information
    vehicleType: 'Тип транспорта',
    make: 'Марка',
    model: 'Модель',
    bodyType: 'Тип кузова',
    color: 'Цвет',
    // Tech Passport
    techPassportSeries: 'Серия техпаспорта',
    licensePlate: 'Государственный номер',
    modelName: 'Модель',
    colorField: 'Цвет',
    // Company
    company: 'ФИРМА',
    // Owner Personal Information
    ownerInfo: 'ФИО',
    firstName: 'Имя',
    lastName: 'Фамилия',
    fatherName: 'Отчество',
    // Owner Address
    address: 'Адрес',
    country: 'Страна',
    province: 'Область',
    cityDistrict: 'Город / Район',
    administrativeArea: 'Административная зона',
    settlement: 'Населенный пункт',
    neighborhood: 'Махалля',
    street: 'Улица и дом',
    companyTaxId: 'СТИР фирмы',
    pinfl: 'ПИНФЛ',
    // Vehicle Specifications
    year: 'Год выпуска',
    type: 'Тип',
    vehicleTypeLight: 'Легковой',
    bodyTypePlaceholder: 'Седан',
    grossWeight: 'Полная масса',
    unladenWeight: 'Снаряженная масса',
    fuelType: 'Тип топлива',
    seatingCapacity: 'Количество мест с водителем',
    // Photo labels
    techPassportFront: 'Фото передней стороны техпаспорта:',
    techPassportBack: 'Фото задней стороны техпаспорта:',
    vehicleFront: 'Фото передней стороны транспорта:',
    vehicleBack: 'Фото задней стороны транспорта:',
    vehicleRight: 'Фото правой стороны транспорта:',
    vehicleLeft: 'Фото левой стороны транспорта:',
    vehicleAngle45: 'Общий вид транспорта 45°:',
    vehicleInterior: 'Фото салона транспорта:',
    // Select placeholders
    select: 'Выбрать',
    // Modal titles
    selectVehicleType: 'Выберите тип транспорта',
    selectMake: 'Выберите марку',
    selectModel: 'Выберите модель',
    selectBodyType: 'Выберите тип кузова',
    selectColor: 'Выберите цвет',
  },

  // Driver Taxi License Screen
  driverTaxiLicense: {
    title: 'ЛИЦЕНЗИЯ',
    subtitle: 'РЕДАКТИРОВАНИЕ ДАННЫХ',
    description: '(Все данные вводятся в соответствии с лицензией)',
    backButton: 'Назад',
    licenseNumber: 'Номер ЛИЦЕНЗИИ',
    issueDate: 'Дата выдачи',
    registryNumber: 'Номер в реестре ЛИЦЕНЗИИ',
    licensePhoto: 'Фото ЛИЦЕНЗИИ:',
    licenseSheetTitle: 'Лист ЛИЦЕНЗИИ',
    licenseSheetNumber: 'Номер листа ЛИЦЕНЗИИ',
    validFrom: 'Срок действия (с)',
    validUntil: 'Срок действия (по)',
    licenseSheetPhoto: 'Фото листа ЛИЦЕНЗИИ:',
    selfEmploymentTitle: 'Самостоятельная занятость',
    selfEmploymentNumber: '№ самостоятельной занятости',
    selfEmploymentPhoto: 'Фото самостоятельной занятости:',
    powerOfAttorneyPhoto: 'Фото доверенности:',
    insurancePhoto: 'Фото страховки:',
    chooseImageOrFile: 'Выбрать изображение или файл',
    howToSelectImage: 'Как вы хотите выбрать изображение или PDF файл?',
    save: 'Сохранить',
    ready: 'Готово',
    cameraPermissionMessage: 'Разрешите использование камеры',
    galleryPermissionMessage: 'Разрешите выбор изображения из галереи',
  },

  // Edit Profile Menu
  editProfile: {
    personalInfo: 'Личные данные',
    passport: 'Данные паспорта',
    license: 'Водительские права',
    vehicle: 'Транспортное средство',
    taxiLicense: 'Лицензия такси',
    driverType: 'Тип водителя',
  },

  // Notifications
  notifications: {
    title: 'Уведомления',
    noNotifications: 'Нет уведомлений',
    markAllRead: 'Отметить все как прочитанные',
    markAllReadConfirm: 'Вы хотите отметить все уведомления как прочитанные?',
    allMarkedRead: 'Все уведомления отмечены как прочитанные',
    markReadError: 'Не удалось отметить уведомление как прочитанное',
    markAllReadError: 'Не удалось отметить все уведомления как прочитанные',
    delete: 'Удалить',
    deleteConfirm: 'Вы хотите удалить это уведомление?',
    deleted: 'Уведомление удалено',
    deleteError: 'Не удалось удалить уведомление',
    loadError: 'Не удалось загрузить уведомления',
    loadErrorDescription: 'Произошла ошибка при загрузке уведомлений',
    justNow: 'Только что',
    minutesAgo: '{count} минут назад',
    hoursAgo: '{count} часов назад',
    daysAgo: '{count} дней назад',
    noNotificationsDescription: 'Новые уведомления появятся здесь',
  },

  // T-018 — новые поля заказа пассажира (только отображение)
  passengerOfferExtras: {
    urgent: 'Срочно',
    arriveBy: 'нужно прибыть до',
    seatsFront: 'Перед',
    seatsBack: 'Зад',
    wholeSalon: 'Весь салон',
    backSalonFull: 'Задний салон полностью',
    positionAny: 'Место без разницы',
    priceNegotiable: 'Цена договорная',
    classStandard: 'Стандарт',
    classComfort: 'Комфорт',
    classBusiness: 'Бизнес',
    classEconom: 'Эконом',
    classTourist: 'Туристический',
    paymentCash: 'Наличные',
    paymentClickPayme: 'Click / Payme',
    paymentFriend: 'Платит друг',
    womanInCar: 'Авто, где есть женщина',
    roofRack: 'Багажник на крыше',
    trailer: 'Прицеп',
    pets: 'С животным',
    baggage: 'Есть багаж',
    roadPickup: 'Выйдет на трассу',
    specialOrder: 'Спецзаказ',
    fixedPrice: 'Фиксированная цена',
    reviewsOffers: 'Рассмотрит предложения',
    waitingFee: 'Ожидание',
    somPerMinute: 'сум/мин',
    freeWaiting: 'бесплатное ожидание {count} мин',
  },

  // Driver Offers
  driverOffers: {
    title: 'Мои объявления',
    createOffer: 'Создать объявление',
    noOffers: 'У вас пока нет объявлений',
    noOffersDescription: 'Создайте новое объявление, чтобы найти пассажиров',
    status: {
      published: 'Активно',
      active: 'Активно',
      archived: 'В архиве',
      cancelled: 'Отменено',
    },
    cancel: 'Отменить',
    confirmCancel: 'Вы хотите отменить объявление?',
    confirmCancelMessage: 'Отмененные объявления можно активировать позже',
    cancelSuccess: 'Объявление успешно отменено',
    from: 'Откуда',
    to: 'Куда',
    departure: 'Время отправления',
    seats: 'Места',
    price: 'Цена',
    pricePerSeat: 'За место',
    note: 'Примечание',
    edit: 'Редактировать',
    delete: 'Удалить',
    publish: 'Опубликовать',
    archive: 'В архив',
    confirmDelete: 'Вы хотите удалить объявление?',
    confirmArchive: 'Вы хотите архивировать объявление?',
    confirmPublish: 'Вы хотите опубликовать объявление?',
    deleteSuccess: 'Объявление успешно удалено',
    archiveSuccess: 'Объявление успешно архивировано',
    publishSuccess: 'Объявление успешно опубликовано',
    rejectionReason: 'Причина отклонения',
    viewDetails: 'Подробности',
    details: 'Детали',
    stop: 'Остановка',
    viewPassengers: 'Просмотр пассажиров',
  },

  // Offer Wizard
  offerWizard: {
    selectCountry: "Выберите страну",
    selectProvince: "Выберите область",
    selectCity: "Выберите город/район",
    title: 'Создать новое объявление',
    editTitle: 'Редактировать объявление',
    fromLabel: 'Откуда',
    fromPlaceholder: 'Точка отправления',
    toLabel: 'Куда',
    toPlaceholder: 'Пункт назначения',
    departLabel: 'Время выезда',
    departCrumb: 'Дата и интервал времени',
    arriveLabel: 'Время прибытия',
    arriveCrumb: 'Дата и время',
    arriveHelper: 'Необязательно. Укажите, если время прибытия важно.',
    arriveNone: 'Не указано',
    urgentDeparture: 'Прямо сейчас (выезжаю по заполнении)',
    nextDay: 'следующий день',
    errorArriveBeforeDeparture:
      'Прибытие не может быть раньше самого позднего времени выезда',
    selectPlace: 'Выберите место',
    swapRoute: 'Поменять направление',
    stopLabel: 'Остановка',
    stopsLabel: 'Промежуточные остановки (необязательно)',
    addStop: 'Добавить',
    stop: 'Остановка',
    stopsHelper: 'Вы можете добавить остановки для посадки пассажиров по пути',
    selectMultiple: 'Выбрать несколько',
    selectCities: 'Выбрать города',
    dateLabel: 'Дата',
    timeLabel: 'Время',
    seatsLabel: 'Количество мест',
    seatsDescription: 'От 1 до 8 мест',
    priceLabel: 'Цена за место',
    currencyLabel: 'Валюта',
    // T-079 / T-080 — amenities, jo'natma, road pickup and the fill-up flag.
    amenitiesLabel: 'Удобства',
    amenityAirCon: 'Кондиционер',
    amenityWifi: 'Интернет, WiFi',
    amenityRoofRack: 'Есть багажник на крыше',
    amenityTrailer: 'Есть прицеп',
    parcelAccepted: 'Принимаю посылки (почту)',
    parcelPriceLabel: 'Цена посылки (сум)',
    parcelMaxKgLabel: 'Максимум (кг)',
    parcelMaxKgHelper: 'Например 20 — принимаю посылки до 20 кг.',
    roadPickupLabel: 'Беру только со стоянки или если выйдете на трассу',
    roadPickupPlaceholder: 'Например: с рынка Куйлюк или если выйдете на дорогу в сторону Ферганы',
    departsWhenFullLabel: 'Еду, как только заполнится (срочно)',
    departsWhenFullHelper: 'Без точного времени — выезжаете, как только займут все места.',
    // T-078 — the mockup's Narxlar list, payment and class.
    pricesSectionTitle: 'Цены',
    priceBackSalonLabel: 'Цена за задний салон (необязательно)',
    priceBackSalonHelper: 'Если весь задний ряд бронирует один человек',
    priceWholeSalonLabel: 'Цена за весь салон (необязательно)',
    priceWholeSalonHelper: 'Если всю машину бронирует один человек',
    waitingFeeLabel: 'Плата за ожидание (сум/мин, необязательно)',
    waitingFeeHelper: 'Показывается пассажиру. Приложение не считает и не списывает эту сумму.',
    freeWaitingLabel: 'Бесплатное ожидание (мин)',
    freeWaitingHelper: 'После этого начинается плата за ожидание. Можно указать 0.',
    pickupFeeLabel: 'Цена подачи (сум)',
    pickupFeeHelper: 'Укажите 0 — подача бесплатная.',
    paymentLabel: 'Способ оплаты',
    paymentCash: 'Наличные',
    paymentCard: 'Click, Payme',
    vehicleClassLabel: 'Класс авто',
    vehicleClass_standard: 'Стандарт',
    vehicleClass_comfort: 'Комфорт',
    vehicleClass_business: 'Бизнес',
    vehicleClass_econom: 'Эконом',
    vehicleClass_tourist: 'Туристический',

    noteLabel: 'Примечание (необязательно)',
    notePlaceholder: 'Например: Багажник полный, в пути не останавливаемся, нужно успеть в аэропорт',
    vehicleLabel: 'Транспорт',
    selectVehicle: 'Выбрать транспорт',
    noVehicle: 'Транспорт не найден',
    addVehicle: 'Добавить транспорт',
    summary: 'Сводка',
    route: 'Маршрут',
    departureTime: 'Время отправления',
    totalSeats: 'Всего мест',
    pricePerSeat: 'Цена за место',
    createOffer: 'Создать объявление',
    updateOffer: 'Обновить объявление',
    saveDraft: 'Сохранить как черновик',
    creating: 'Создание...',
    updating: 'Обновление...',
    createSuccess: 'Объявление успешно создано',
    updateSuccess: 'Объявление успешно обновлено',
    errorMinAdvance: 'Время отправления должно быть не менее чем через 30 минут',
    errorInvalidSeats: 'Количество мест должно быть от 1 до 8',
    errorInvalidPrice: 'Цена должна быть не менее 5000',
    errorMissingFields: 'Пожалуйста, заполните все обязательные поля',
  },

  // Offer Passengers
  offerPassengers: {
    title: 'Запросы пассажиров',
    all: 'Все',
    pending: 'Ожидает',
    confirmed: 'Подтверждено',
    rejected: 'Отклонено',
    cancelled: 'Отменено',
    confirm: 'Подтвердить',
    reject: 'Отклонить',
    confirmPassenger: 'Подтвердить пассажира',
    rejectPassenger: 'Отклонить пассажира',
    rejectReasonOptional: 'Укажите причину (необязательно):',
    rejectReasonPlaceholder: 'например, Мест нет',
    message: 'Сообщение',
    rejectionReason: 'Причина отклонения',
    confirmSuccess: 'Пассажир успешно подтвержден',
    rejectSuccess: 'Пассажир отклонен',
    noPassengers: 'Пассажиров пока нет',
    noPassengersMessage: 'Пассажиры появятся здесь, когда присоединятся к вашему объявлению',
    noPassengersFiltered: 'Нет пассажиров с этим статусом',
    noPassengersFilteredMessage: 'Нет запросов пассажиров с этим статусом',
    seatsRequested: 'Запрошено {count} мест',
    seatsRequestedOne: 'Запрошено 1 место',
    frontSeatRequested: 'Запрошено переднее сиденье',
    agreedPriceFrontSeat: 'Согласовано: {price} {currency} (переднее сиденье)',
    agreedPriceMultiple: 'Согласовано: {total} {currency} ({seats} мест, 1 переднее сиденье)',
    agreedPriceCalculation: 'Согласовано: {price} {currency} × {seats} = {total} {currency}',
    unknownPassenger: 'Неизвестно',
    confirmPassengerMessage: 'Подтвердить {name} для {seats} мест?',
    confirmPassengerMessageOne: 'Подтвердить {name} для 1 места?',
    contactTitle: 'Связь с пассажиром',
    noPhone: 'У пассажира не указан номер телефона',
  },
  /** T-101 step 15 — see uz.ts for the artboard the labels come from. */
  drawer: {
    open: "Меню",
    close: "Закрыть меню",
    soon: "Скоро",
    groupOffers: "Объявления",
    offerCreate: "Создать объявление",
    offerMine: "Мои объявления",
    groupOrders: "Заказы",
    ordersIncoming: "Входящие заказы",
    ordersMine: "Мои заказы",
    cars: "Мои автомобили",
    messages: "Сообщения",
    balance: "Баланс",
    income: "Доходы и расходы",
    profile: "Мой профиль",
    groupDocs: "Мои документы",
    docsAll: "Все документы",
    docsPassport: "Паспорт",
    docsLicense: "Водительское удостоверение",
    docsTechPassport: "Тех.паспорт",
    docsTaxiLicense: "Лицензия",
    settings: "Настройки",
    guides: "Инструкции",
  },

  // T-101 step 17 — the passenger-orders screen (DriverQidiruv). 17c: the card.
  // T-101 step 18 — the driver's rides screen (`DriverMyOrder`).
  myRides: {
    phaseJarayon: 'В процессе',
    phaseFaol: 'Активные заказы',
    phaseTarix: 'История заказов',
    emptyJarayon: 'Нет рейсов в процессе',
    emptyFaol: 'Нет активных рейсов',
    emptyTarix: 'История заказов пуста',
    stateCollecting: 'Набирается',
    stateConfirmed: 'Подтверждён',
    stateDone: 'Завершён',
    fillFull: 'Заказ заполнен',
    fillOpen: 'Есть свободные места',
    incomeExpected: 'ожидаемый доход',
    incomeEarned: 'полученный доход',
    statSeats: 'занято мест',
    statOrders: 'Заказы',
    pastHint: 'Время отправления прошло. Завершите рейс, отправив его в архив.',
    cancelRide: 'Отменить рейс',
    bookedAt: 'Время заказа',
    bookingTitle: 'Детали заказа',
    passengersHeading: "Пассажиры этого рейса",
    ordersCount: "Заказов: {count}",
    reasonCarBroken: 'Моя машина сломалась.',
    reasonCarNotRunning: 'Моя машина не едет.',
    reasonNoFuel: 'Не смог заправиться.',
    reasonPassengerLied: 'Пассажир обманул.',
    reasonNoAnswer: 'Пассажир не ответил на звонок.',
    reasonNotLeftHome: 'Пассажир не вышел из дома.',
    reasonNotAtAddress: 'Пассажир не вышел по указанному адресу.',
    reasonWrongAddress: 'Пассажир выбрал неверный адрес.',
    reasonOther: 'Другое',
  },
  passengerOrders: {
    priceListed: "Цена объявления",
    pricePassenger: "Цена пассажира",
    priceMine: "Ваше предложение",
    kindOddiy: "Обычный",
    kindMaxsus: "Особый (платный)",
    people: "чел.",
    // 17d: the screen
    title: "Междугородние",
    kindTaxi: "Такси",
    kindParcel: "Посылка",
    modeIncoming: "Входящие заказы",
    modeSent: "Отправить предложение",
    tabAll: "Все",
    tabOddiy: "Обычные",
    tabMaxsus: "Особый заказ",
    sortMatch: "Подходящие",
    sortPrice: "Цена",
    sortSeats: "Пассажиры",
    sortSoon: "Ближайшие",
    titleIncoming: "Заказы от пассажиров",
    titleOddiy: "Обычные заказы — по цене объявления",
    titleMaxsus: "Особые (платные) — по цене пассажира",
    titleSent: "Отправленные предложения",
    countOrders: "заказ(ов)",
    countOffers: "предложени(й)",
    emptyIncoming: "Входящих заказов нет",
    emptySent: "Отправленных предложений нет",
    anyRoute: "Все направления",
    // 17e: the sheet
    sheetIncoming: "Заказ пассажира",
    sheetSent: "Ваше предложение",
    code: "Заказ",
    seatsTitle: "Запрошенные места",
    seatKind: "Тип места",
    specialTitle: "Цены особого заказа",
    payment: "Оплата",
    phone: "Телефон",
    phoneHidden: "Откроется после подтверждения пассажиром",
    accept: "Принять",
    offer: "Отправить предложение",
    yourPrice: "Ваша цена",
    thousands: "000",
    sendOffer: "Отправить предложение",
    notePlaceholder: "Комментарий: заберу по пути, большой багажник...",
    acceptConfirmTitle: "Принять заказ?",
    acceptConfirmBody: "Пассажиру будет отправлено предложение по цене объявления. Телефон откроется после подтверждения. Итого:",
    // 17f: the result dialog
    resultAcceptTitle: "Заказ принят",
    resultAcceptBody: "Пассажиру отправлено предложение по цене объявления. Когда он подтвердит, откроется телефон и придёт уведомление.",
    resultOfferBody: "Пассажир рассмотрит предложение. Когда придёт ответ, вы получите уведомление.",
    resultOfferPrice: "Цена предложения",
    resultTotal: "Итого к оплате",
    close: "Закрыть",
  },
};
