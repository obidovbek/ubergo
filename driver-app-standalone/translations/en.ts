/**
 * English Translations
 */

export default {
  // T-054 — shared by utils/contactPhone.ts, which any screen showing a
  // confirmed counterparty's number can use.
  contact: {
    noPhone: 'No phone number on file',
    dialFailed: 'Could not start the call',
    noEmail: 'No email address on file',
    emailFailed: 'Could not open the mail app',
  },
  common: {
    continue: 'Continue',
    cancel: 'Cancel',
    delete: 'Delete',
    confirm: 'Confirm',
    ok: 'OK',
    error: 'Error',
    success: 'Success',
    loading: 'Loading...',
    retry: 'Retry',
    back: 'Back',
    next: 'Next',
    saving: 'Saving...',
    datePlaceholder: 'DD.MM.YYYY',
    selectDate: 'Select date',
    day: 'Day',
    month: 'Month',
    year: 'Year',
    // Used by the shared modal components (T-036) — kept under `common` so the copies
    // in both apps stay identical and independent of any screen's namespace.
    search: 'Search...',
    noResults: 'Nothing found',
    hour: 'Hour',
    minute: 'Minute',
    camera: 'Camera',
    gallery: 'Gallery',
    // T-057 — the PhotoSourceModal's fallback heading. Screens with their own
    // wording (driverLicense.selectPhoto, …) pass it explicitly.
    selectPhoto: 'Select a photo',
    // T-057 — three screens had these hard-coded in Uzbek, so a Russian or
    // English user was shown Uzbek. The two older screens keep their own
    // screen-scoped copies, which were already translated.
    cameraPermissionMessage: 'Please allow access to the camera',
    galleryPermissionMessage: 'Please allow access to the gallery',
    permissionRequired: 'Permission required',
    // `all`/`viewAll` existed in uz only — the i18n check caught it when the
    // T-037 filter bar asked for `common.all` (T-037 step 5).
    viewAll: 'View all',
    all: 'All',
    monthJanuary: 'January',
    monthFebruary: 'February',
    monthMarch: 'March',
    monthApril: 'April',
    monthMay: 'May',
    monthJune: 'June',
    monthJuly: 'July',
    monthAugust: 'August',
    monthSeptember: 'September',
    monthOctober: 'October',
    monthNovember: 'November',
    monthDecember: 'December',
    info: 'Info',
  },


  splash: {
    appName: 'UbexGo',
    tagline: 'Drive & Earn',
    loading: 'Loading...',
  },

  auth: {
    appName: 'UbexGo',
    // T-061 — see the uz file: a brand word, deliberately untranslated.
    appNameDriverSuffix: 'Driver',
    logout: 'Logout',
    accountBlocked: 'Account Blocked',
    accountBlockedDescription: 'Unfortunately, your account has been blocked. Please contact support or try again later.',
    accountPendingDelete: 'Account Deletion in Progress',
    accountPendingDeleteDescription: 'Your account is in the process of being deleted. If this is a mistake, please contact support.',
    accountSuspended: 'Account Suspended',
    accountSuspendedDescription: 'Your account has been temporarily suspended. Please contact support.',
    contactSupport: 'Contact Support',
    supportEmail: 'Email: support@ubexgo.uz',
    supportPhone: 'Phone: +998 XX XXX XX XX',
    emailNotAvailable: 'Email client is not available on this device',
    emailError: 'Unable to open email client',
    phoneNotAvailable: 'Phone dialer is not available on this device',
    phoneError: 'Unable to open phone dialer',
  },

  searchPassengerOffers: {
    from: 'From',
    to: 'To',
    filter: 'Filters',
    applyFilters: 'Apply filters',
    loginRequired: 'Log in to view this order',
    // T-038/T-037 step 2: this screen was almost entirely hard-coded English —
    // it had never been reachable, so nobody had seen it.
    title: 'Passenger requests',
    fromLabel: 'FROM',
    toLabel: 'TO',
    clear: 'Clear',
    maxBudget: 'Max budget',
    perSeat: 'per seat',
    viewDetails: 'View details',
    loading: 'Loading passenger requests...',
    emptySelectTitle: 'Select locations',
    emptySelectSubtitle: 'Choose departure and destination to find passenger requests',
    emptyNoneTitle: 'No passenger requests',
    emptyNoneSubtitle: 'No passengers are looking for rides on this route right now',
    resultsCount: '{count} orders',
    sortBy: 'Sort by',
    sortDateAsc: 'Date (earliest first)',
    sortPriceDesc: 'Budget (highest first)',
    sortPriceAsc: 'Budget (lowest first)',
    sortSeatsDesc: 'Seats (most first)',
    filtersTitle: 'Filters',
    maxBudgetLabel: 'Maximum budget (per seat)',
    maxBudgetPlaceholder: 'Any amount',
    minSeatsLabel: 'Minimum seats needed',
    minSeatsPlaceholder: 'Any number',
    clearAllFilters: 'Clear all filters',
    // T-049: the offer card used to render hard-coded English ("2 seats needed").
    seatsNeededCount: '{count} seats needed',
    // T-049: eight more hard-coded English toasts lived in this screen's geo
    // pickers. They only fire on an error or an out-of-order tap, which is why
    // nobody had seen them.
    loadProvincesFailed: 'Failed to load provinces',
    loadCitiesFailed: 'Failed to load cities',
    selectCountryFirst: 'Please select a country first',
    selectProvinceFirst: 'Please select a province first',
  },

  // Passenger order details (driver side) — T-037
  passengerOfferDetails: {
    title: 'Order details',
    from: 'FROM',
    to: 'TO',
    departure: 'Departure',
    passenger: 'Passenger',
    passengerUnknown: 'Not specified',
    seatsNeeded: 'Seats needed',
    maxPrice: 'Max price',
    priceNotSet: 'Not specified',
    note: 'Note',
    notFound: 'Order not found',
    loadFailed: 'Failed to load the order',
    takeOrder: "I'll take this order",
    joinTitle: 'Send your offer',
    vehicle: 'Vehicle',
    noVehicle: 'You have no vehicle added. Add a vehicle in your profile first.',
    seatsOffered: 'Seats offered',
    seatsHint: 'The passenger needs at least {count} seats',
    pricePerSeat: 'Price per seat',
    total: 'Total',
    messageLabel: 'Message (optional)',
    messagePlaceholder: 'Message to the passenger...',
    send: 'Send',
    sending: 'Sending...',
    sentTitle: 'Offer sent',
    sentMessage: 'Wait for the passenger to respond',
    alreadySent: 'Your offer has been sent',
    statusConfirmed: 'Your offer was confirmed',
    statusRejected: 'Your offer was rejected',
    statusCancelled: 'You cancelled your offer',
    joinFailed: 'Failed to send the offer',
    errorPrice: 'Enter a valid price',
    errorSeats: 'Not enough seats',
  },

  // Offers the driver has sent on passenger orders — T-037
  myJoinRequests: {
    title: 'My sent offers',
    empty: 'You have not sent any offers yet',
    loadFailed: 'Failed to load your offers',
    offerGone: 'Order unavailable',
    pricePerSeat: 'Price per seat',
    seatsOffered: 'Seats offered',
    total: 'Total',
    status_pending: 'Pending',
    status_confirmed: 'Confirmed',
    status_rejected: 'Rejected',
    status_cancelled: 'Cancelled',
    cancel: 'Cancel offer',
    cancelTitle: 'Cancel this offer?',
    cancelWarning:
      'Once cancelled you cannot offer on this order again. This cannot be undone.',
    cancelConfirm: 'Yes, cancel it',
    cancelled: 'Offer cancelled',
    cancelFailed: 'Failed to cancel',
    contactTitle: 'Contact the passenger',
    noPhone: 'The passenger has no phone number on file',
  },

  phoneRegistration: {
    brand: 'UbexGo Driver',
    selectCountry: 'Select country',
    title: 'Driver registration',
    driverType: 'Driver',
    instruction: 'Enter your phone number registered in UbexGo app',
    orIdText: 'or your UbexGo ID number',
    termsText: 'By clicking "Continue" I accept the ',
    termsLink: 'user agreement',
    termsText2: ' terms',
    phonePlaceholder: '00 000-00-00',
    googleSignIn: 'Sign in with Google',
    appleSignIn: 'Continue with Apple',
    facebookSignIn: 'Login with Facebook',
    microsoftSignIn: 'Sign in with Microsoft',
    signingIn: 'Signing in...',
    footerText: 'By registering with the above platforms I accept ',
    footerLink: 'terms of use',
    footerText2: '',
    alreadyHaveAccount: 'Already have an account? ',
    login: 'Login',
    errorPhoneIncomplete: 'Please enter your complete phone number (9 digits)',
    errorGoogleNotReady: 'Google service is not ready. Please try again.',
    errorGoogleCancelled: 'Google sign-in was cancelled',
    errorNetwork: 'Network connection problem. Please try again.',
    errorGoogleFailed: 'Google sign-in failed',
    infoNotAvailable: ' sign-in is not available yet',
  },

  // Register First Screen
  registerFirst: {
    title: 'Please Register in User App First',
    description: 'To register as a driver, you must first register in the UbexGo user app with your phone number.',
    step1: 'Download the UbexGo user app',
    step2: 'Register in the app with your phone number',
    step3: 'Then return to the driver app and continue with the same phone number',
    downloadAndroid: 'Download for Android',
    downloadIOS: 'Download for iOS',
    noUrlsAvailable: 'App store URLs are not configured yet',
  },

  otpVerification: {
    brand: 'UbexGo Driver',
    title: 'Enter code',
    subtitle: 'Enter the verification code received in your UbexGo app account',
    verifying: 'Verifying...',
    resendQuestion: "Didn't receive the code? ",
    resendLink: 'Resend',
    resendIn: 'Resend in {seconds}s',
    newCodeSent: 'New code sent via push notification',
    errorResendPush:
      'Failed to send the push notification. Make sure the passenger app is open.',
    errorIncomplete: 'Please enter the complete verification code',
    errorIncorrect: 'Incorrect code. Remaining attempts: ',
    errorNoAttempts: 'Error',
    errorNoAttemptsMessage: 'Attempts exhausted. Please try again',
    errorResend: 'Error sending code',
    phoneVerified: 'Phone verified',
    attemptsLabel: 'Attempt: ',
    remainingLabel: ' | Remaining: ',
  },

  userDetails: {
    title: 'Intercity',
    subtitle: 'Fill out the form to get\nbonuses and gifts',
    requiredField: '* - required field',
    firstName: 'FIRST NAME',
    lastName: 'Last Name',
    fatherName: "Father's Name",
    gender: 'Gender',
    male: 'Male',
    female: 'Female',
    birthDate: 'Date of Birth',
    email: 'email',
    phone: 'Phone:',
    userId: 'ID:',
    promoCode: 'PROMO:',
    additionalPhones: 'Enter additional numbers\nto stay in touch\n(up to 5. In case of signal loss....)',
    submit: 'Done',
    firstNamePlaceholder: 'Enter your first name',
    lastNamePlaceholder: 'Enter your last name',
    fatherNamePlaceholder: "Your father's name",
    emailPlaceholder: 'example@email.com',
    userIdPlaceholder: '1 001 117',
    promoCodePlaceholder: '1515',
    birthDatePlaceholder: '21.01.1985',
    phonePlaceholder: '90 123 45 67',
    genderHelper: 'To find suitable transport for you',
    birthDateHelper: 'For a gift from the company on your birthday',
    emailHelper: 'To recover your account',
    infoBoxText: 'Person who invited you (sent link) ',
    infoBoxPhone: 'phone number',
    infoBoxId: '\nID number',
    infoBoxPromo: '\nor promo ',
    infoBoxPromoCode: 'PROMO code',
    infoBoxPromo2: ' to get\n',
    infoBoxBonus: '10 000',
    infoBoxBonus2: ' bonus',
    selectDate: 'Select date of birth',
    day: 'Day',
    month: 'Month',
    year: 'Year',
    errorFirstName: 'First name is required',
    errorLastName: 'Last name is required',
    errorGender: 'Select gender',
    errorBirthDate: 'Enter date of birth',
    errorBirthDateIncomplete: 'Enter complete date',
    errorEmail: 'Email is required',
    errorEmailInvalid: 'Invalid email format',
    errorPhone: 'Please enter complete phone number',
    errorAllFields: 'Please fill all required fields',
    successTitle: 'Success!',
    successMessage: 'Registration completed successfully',
    errorUpdate: 'Error updating profile',
  },

  menu: {
    /** T-101: the top bar screen name. `title` is the CARD heading. */
    screenTitle: 'Intercity',
    title: 'TAXI',
    subtitle: 'Select country',
    uzbekistan: 'Uzbekistan',
    guest: 'Guest',
    // `myOffers` was rendered from uz only — ru/en showed the raw key on the
    // home screen. Found and fixed during T-037 step 1.
    myOffers: 'My offers',
    passengerOrders: 'Passenger orders',
    myJoinRequests: 'My sent offers',
    viloyatlar: 'Taxi\nIntercity',
    ichi: 'Taxi\nWithin\nCity',
    tuman: 'Taxi\nDistrict,\nlocal and nearby\ndistances',
    empty: 'Taxi',
    xalqaro: 'Taxi\nINTERNATIONAL',
  },

  profile: {
    logout: 'Logout',
    logoutConfirm: 'Are you sure you want to logout?',
    logoutError: 'Error logging out. Please try again.',
    noEmail: 'No email',
    noPhone: 'No phone',
    driverId: 'Driver ID',
    appVersion: 'UbexGo v1.0.0',
    notifications: 'Notifications',
    editProfile: 'Edit Profile',
    paymentMethods: 'Payment Methods',
    tripHistory: 'Trip History',
    helpSupport: 'Help & Support',
    settings: 'Settings',
    title: 'Profile',
    myOffers: 'My offers',
  },

  months: {
    january: 'January',
    february: 'February',
    march: 'March',
    april: 'April',
    may: 'May',
    june: 'June',
    july: 'July',
    august: 'August',
    september: 'September',
    october: 'October',
    november: 'November',
    december: 'December',
  },

  // Driver Messages
  driver: {
    profileUpdated: 'Profile updated',
    profileIncomplete: 'Profile incomplete',
    registrationComplete: 'Registration complete',
    passportUpdated: 'Passport information saved',
    licenseUpdated: 'License information saved',
    vehicleUpdated: 'Vehicle information saved',
    taxiLicenseUpdated: 'Taxi license saved',
  },

  // Backend Error Messages
  errors: {
    network: 'No internet connection',
    timeout: 'Request timeout',
    serverError: 'Server error occurred',
    unauthorized: 'Unauthorized',
    forbidden: 'Access forbidden',
    notFound: 'Not found',
    badRequest: 'Bad request',
    conflict: 'Data already exists',
    validation: 'Invalid data',
    unknown: 'Unknown error occurred',
    tryAgain: 'Please try again',
    tooManyRequests: 'Too many requests. Please try again later',
    // T-035 — recovered from the overridden block; 9 call sites across both apps.
    loadFailed: 'Failed to load data',
    saveFailed: 'Failed to save data',
    deleteFailed: 'Failed to delete',
    updateFailed: 'Failed to update',
    createFailed: 'Failed to create',
  },

  // Network Status
  networkStatus: {
    noInternet: 'No Internet Connection',
    noInternetMessage: 'We can\'t connect to the server. Please check your internet connection to continue receiving ride requests.',
    hint: 'Make sure Wi-Fi or mobile data is turned on',
    retry: 'Try Again',
    tip1: 'Check your Wi-Fi or mobile data settings',
    tip2: 'Pull down to refresh once connected',
  },

  // Form Validation Messages
  formValidation: {
    // Driver Details
    driverTypeRequired: 'Please select driver type',
    driverTypeInvalid: 'Invalid driver type selected',

    // Personal Info
    firstNameRequired: 'First name is required',
    firstNameTooShort: 'First name is too short',
    lastNameRequired: 'Last name is required',
    lastNameTooShort: 'Last name is too short',
    fatherNameRequired: "Father's name is required",
    genderRequired: 'Please select gender',
    genderInvalid: 'Invalid gender selected',
    birthDateRequired: 'Date of birth is required',
    birthDateInvalid: 'Invalid date format',
    emailInvalid: 'Invalid email format',
    addressCountryRequired: 'Country is required',
    addressRegionRequired: 'Region is required',
    addressCityRequired: 'City is required',
    photoRequired: 'Photo upload is required',

    // Passport Info
    idCardRequired: 'ID card number is required',
    idCardTooShort: 'ID card number is too short',
    pinflRequired: 'PINFL is required',
    pinflInvalid: 'PINFL must be 14 digits',
    citizenshipRequired: 'Citizenship is required',
    issueDateRequired: 'Issue date is required',
    expiryDateRequired: 'Expiry date is required',

    // License Info
    licenseNumberRequired: 'License number is required',
    licenseNumberTooShort: 'License number is too short',
    issueDateInvalid: 'Invalid issue date',
    categoryRequired: 'At least one category must be selected',

    // Emergency Contacts
    phoneRequired: 'Phone number is required',
    phoneInvalid: 'Invalid phone format',
    relationshipRequired: 'Relationship is required',

    // Vehicle Info
    vehicleTypeRequired: 'Vehicle type is required',
    vehicleTypeInvalid: 'Invalid vehicle type',
    vehicleMakeRequired: 'Please select make',
    vehicleModelRequired: 'Please select model',
    vehicleBodyTypeRequired: 'Please select body type',
    vehicleColorRequired: 'Please select color',
    licensePlateRequired: 'License plate is required',
    licensePlateTooShort: 'License plate must be at least 3 characters',
    techPassportSeriesRequired: 'Technical passport series is required',
    techPassportSeriesTooShort: 'Technical passport series must be at least 3 characters',
    makeRequired: 'Make is required',
    modelRequired: 'Model/Type is required',
    yearRequired: 'Year is required',
    yearInvalid: 'Invalid year format',
    yearOutOfRange: 'Year must be between 1900 and {maxYear}',
    colorRequired: 'Color is required',
    ownerFirstNameRequired: 'Owner first name is required',
    ownerFirstNameTooShort: 'Owner first name must be at least 2 characters',
    ownerLastNameRequired: 'Owner last name is required',
    ownerLastNameTooShort: 'Owner last name must be at least 2 characters',
    ownerFatherNameRequired: "Owner father's name is required",
    ownerFatherNameTooShort: "Owner father's name must be at least 2 characters",
    ownerPinflRequired: 'Owner PINFL is required',
    ownerPinflInvalid: 'PINFL must be 14 digits',
    ownerAddressCountryRequired: 'Please select country',
    ownerAddressProvinceRequired: 'Please select province',
    ownerAddressCityDistrictRequired: 'Please select city or district',
    ownerAddressStreetRequired: 'Street address is required',
    ownerAddressStreetTooShort: 'Street address must be at least 5 characters',
    fuelTypesRequired: 'At least one fuel type must be selected',
    grossWeightInvalid: 'Gross weight must be a positive number',
    grossWeightTooLarge: 'Gross weight must not exceed 50000 kg',
    unladenWeightInvalid: 'Unladen weight must be a positive number',
    unladenWeightTooLarge: 'Unladen weight must not exceed 50000 kg',
    seatingCapacityInvalid: 'Seating capacity must be a positive number',
    seatingCapacityTooLarge: 'Seating capacity must not exceed 50',
    companyNameTooShort: 'Company name must be at least 2 characters',

    // Taxi License
    taxiLicenseRequired: 'Taxi license number is required',
    taxiLicenseTooShort: 'License number must be at least 3 characters',
    licenseIssueDateRequired: 'Issue date is required',
    licenseIssueDateInvalid: 'Invalid date format',
    licenseRegistryNumberRequired: 'License registry number is required',
    licenseRegistryNumberTooShort: 'Registry number must be at least 3 characters',
    licenseSheetRequired: 'License sheet number is required',
    licenseSheetTooShort: 'License sheet number must be at least 3 characters',
    validFromRequired: 'Valid from date is required',
    validFromInvalid: 'Invalid valid from date format',
    validUntilRequired: 'Valid until date is required',
    validUntilInvalid: 'Invalid valid until date format',
    validUntilBeforeValidFrom: 'Valid until date must be after valid from date',
    selfEmploymentNumberRequired: 'Self-employment number is required',
    selfEmploymentNumberTooShort: 'Self-employment number must be at least 3 characters',

    // Generic messages
    required: 'This field is required',
    tooShort: 'Value is too short',
    tooLong: 'Value is too long',
    invalid: 'Invalid format',
    mustBeNumber: 'Must be a number',
    mustBePositive: 'Must be a positive value',
    fixErrors: 'Please correct the highlighted errors',
    dateInvalid: 'Invalid date format',
    dateCannotBeFuture: 'The date cannot be later than today',
    dateCannotBePast: 'The date cannot be earlier than today',
  },

  // Driver License Screen
  driverLicense: {
    selectCountryCode: "Select country code",
    title: 'Intercity',
    subtitle: 'DRIVING LICENSE',
    description: '(All information is entered according to passport)',
    issueDate: 'Issue date',
    licenseNumber: 'License number',
    licenseNumberPlaceholder: 'AG 1234567',
    categoryDatePlaceholder: 'Permission granted date',
    licensePhoto: 'Driving license:',
    licensePhotoFrontHint: '(Front side with photo)',
    licensePhotoBackHint: '(Back side)',
    emergencyContactsTitle: 'Enter 2 close relatives phone numbers for emergencies',
    emergencyContactsHint: '(father, mother, brother...)',
    phoneNumberPlaceholder: '0 00 00 00 00',
    relationshipFather: 'Father',
    relationshipSpouse: 'Spouse',
    selectPhoto: 'Select photo',
    howToSelectPhoto: 'How would you like to select the photo?',
    cameraPermissionMessage: 'Please grant permission to use camera',
    galleryPermissionMessage: 'Please grant permission to access gallery',
    photoUploaded: 'Photo uploaded successfully',
    photoUploadError: 'Error uploading photo',
    photoReadError: 'Error reading photo',
    photoSelectError: 'Error selecting photo',
    licenseSaved: 'License information saved',
    errorUpdate: 'Error saving information',
    licenseNumberRequired: 'Please enter license number',
    issueDateRequired: 'Please enter issue date',
  },

  // Driver Passport Screen — see the uz file for why only these three exist.
  driverPassport: {
    birthPlaceCountry: 'Country',
    birthPlaceProvince: 'Province',
    birthPlaceCityDistrict: 'City / District',
  },

  // Driver Vehicle Screen
  driverVehicle: {
    title: 'VEHICLE INFORMATION',
    subtitle: 'EDIT INFORMATION',
    description: '(All information is entered according to technical passport)',
    backButton: 'Back',
    // Vehicle Basic Information
    vehicleType: 'Vehicle Type',
    make: 'Make',
    model: 'Model',
    bodyType: 'Body Type',
    color: 'Color',
    // Tech Passport
    techPassportSeries: 'Tech Passport Series',
    licensePlate: 'License Plate',
    modelName: 'Model/Name',
    colorField: 'Color',
    // Company
    company: 'COMPANY',
    // Owner Personal Information
    ownerInfo: 'FULL NAME',
    firstName: 'First Name',
    lastName: 'Last Name',
    fatherName: 'Father\'s Name',
    // Owner Address
    address: 'Address',
    country: 'Country',
    province: 'Province',
    cityDistrict: 'City / District',
    administrativeArea: 'Administrative Area',
    settlement: 'Settlement',
    neighborhood: 'Neighborhood',
    street: 'Street and House',
    companyTaxId: 'Company Tax ID',
    pinfl: 'PINFL',
    // Vehicle Specifications
    year: 'Manufacturing Year',
    type: 'Type',
    vehicleTypeLight: 'Light',
    bodyTypePlaceholder: 'Sedan',
    grossWeight: 'Gross weight',
    unladenWeight: 'Unladen weight',
    fuelType: 'Fuel Type',
    seatingCapacity: 'Seating capacity (with driver)',
    // Photo labels
    techPassportFront: 'Tech Passport Front Side Photo:',
    techPassportBack: 'Tech Passport Back Side Photo:',
    vehicleFront: 'Vehicle Front Side Photo:',
    vehicleBack: 'Vehicle Back Side Photo:',
    vehicleRight: 'Vehicle Right Side Photo:',
    vehicleLeft: 'Vehicle Left Side Photo:',
    vehicleAngle45: 'Vehicle General View 45°:',
    vehicleInterior: 'Vehicle Interior Photo:',
    // Select placeholders
    select: 'Select',
    // Modal titles
    selectVehicleType: 'Select Vehicle Type',
    selectMake: 'Select Make',
    selectModel: 'Select Model',
    selectBodyType: 'Select Body Type',
    selectColor: 'Select Color',
  },

  // Driver Taxi License Screen
  driverTaxiLicense: {
    title: 'LICENSE',
    subtitle: 'EDIT INFORMATION',
    description: '(All information is entered according to the license)',
    backButton: 'Back',
    licenseNumber: 'LICENSE number',
    issueDate: 'Issue date',
    registryNumber: 'LICENSE registry number',
    licensePhoto: 'LICENSE photo:',
    licenseSheetTitle: 'LICENSE sheet',
    licenseSheetNumber: 'LICENSE sheet number',
    validFrom: 'Valid from',
    validUntil: 'Valid until',
    licenseSheetPhoto: 'LICENSE sheet photo:',
    selfEmploymentTitle: 'Self-employment',
    selfEmploymentNumber: 'Self-employment №',
    selfEmploymentPhoto: 'Self-employment photo:',
    powerOfAttorneyPhoto: 'Power of attorney photo:',
    insurancePhoto: 'Insurance photo:',
    chooseImageOrFile: 'Choose image or file',
    howToSelectImage: 'How do you want to select an image or PDF file?',
    save: 'Save',
    ready: 'Ready',
    cameraPermissionMessage: 'Please grant permission to use the camera',
    galleryPermissionMessage: 'Please grant permission to select image from gallery',
  },

  // Edit Profile Menu
  editProfile: {
    personalInfo: 'Personal Information',
    passport: 'Passport Information',
    license: 'Driving License',
    vehicle: 'Vehicle Information',
    taxiLicense: 'Taxi License',
    driverType: 'Driver Type',
  },

  // Notifications
  notifications: {
    title: 'Notifications',
    noNotifications: 'No notifications',
    markAllRead: 'Mark all as read',
    markAllReadConfirm: 'Do you want to mark all notifications as read?',
    allMarkedRead: 'All notifications marked as read',
    markReadError: 'Failed to mark notification as read',
    markAllReadError: 'Failed to mark all notifications as read',
    delete: 'Delete',
    deleteConfirm: 'Do you want to delete this notification?',
    deleted: 'Notification deleted',
    deleteError: 'Failed to delete notification',
    loadError: 'Failed to load notifications',
    loadErrorDescription: 'An error occurred while loading notifications',
    justNow: 'Just now',
    minutesAgo: '{count} minutes ago',
    hoursAgo: '{count} hours ago',
    daysAgo: '{count} days ago',
    noNotificationsDescription: 'New notifications will appear here',
  },

  // T-018 — new passenger order fields (display only)
  passengerOfferExtras: {
    urgent: 'Urgent',
    arriveBy: 'must arrive by',
    seatsFront: 'Front',
    seatsBack: 'Back',
    wholeSalon: 'Whole salon',
    backSalonFull: 'Entire back salon',
    positionAny: 'Any seat position',
    priceNegotiable: 'Price negotiable',
    classStandard: 'Standard',
    classComfort: 'Comfort',
    classBusiness: 'Business',
    classEconom: 'Econom',
    classTourist: 'Tourist',
    paymentCash: 'Cash',
    paymentClickPayme: 'Click / Payme',
    paymentFriend: 'A friend pays',
    womanInCar: 'Car with a woman in it',
    roofRack: 'Roof rack',
    trailer: 'Trailer',
    pets: 'Has a pet',
    baggage: 'Has baggage',
    roadPickup: 'Will wait on the road',
    specialOrder: 'Special order',
    fixedPrice: 'Fixed price',
    reviewsOffers: 'Will review offers',
    waitingFee: 'Waiting',
    somPerMinute: 'so\'m/min',
    freeWaiting: 'free waiting {count} min',
  },

  // Driver Offers
  driverOffers: {
    title: 'My Offers',
    createOffer: 'Create Offer',
    noOffers: 'You don\'t have any offers yet',
    noOffersDescription: 'Create a new offer to find passengers',
    status: {
      published: 'Active',
      active: 'Active',
      archived: 'Archived',
      cancelled: 'Cancelled',
    },
    cancel: 'Cancel',
    confirmCancel: 'Do you want to cancel the offer?',
    confirmCancelMessage: 'Cancelled offers can be reactivated later',
    cancelSuccess: 'Offer cancelled successfully',
    from: 'From',
    to: 'To',
    departure: 'Departure Time',
    seats: 'Seats',
    price: 'Price',
    pricePerSeat: 'Per Seat',
    note: 'Note',
    edit: 'Edit',
    delete: 'Delete',
    publish: 'Publish',
    archive: 'Archive',
    confirmDelete: 'Do you want to delete the offer?',
    confirmArchive: 'Do you want to archive the offer?',
    confirmPublish: 'Do you want to publish the offer?',
    deleteSuccess: 'Offer deleted successfully',
    archiveSuccess: 'Offer archived successfully',
    publishSuccess: 'Offer published successfully',
    rejectionReason: 'Rejection Reason',
    viewDetails: 'View Details',
    details: 'Details',
    stop: 'Stop',
    viewPassengers: 'View Passengers',
  },

  // Offer Wizard
  offerWizard: {
    selectCountry: "Select country",
    selectProvince: "Select province",
    selectCity: "Select city/district",
    title: 'Create New Offer',
    editTitle: 'Edit Offer',
    fromLabel: 'From',
    fromPlaceholder: 'Starting Point',
    toLabel: 'To',
    toPlaceholder: 'Destination',
    departLabel: 'Departure time',
    departCrumb: 'Date and time range',
    arriveLabel: 'Arrival time',
    arriveCrumb: 'Date and time',
    arriveHelper: 'Optional. Set it only if the arrival time matters.',
    arriveNone: 'Not set',
    urgentDeparture: 'Right now (leaving once full)',
    nextDay: 'next day',
    errorArriveBeforeDeparture:
      'Arrival cannot be earlier than the latest departure time',
    selectPlace: 'Choose a place',
    swapRoute: 'Swap route',
    stopLabel: 'Stop',
    stopsLabel: 'Intermediate Stops (Optional)',
    addStop: 'Add',
    stop: 'Stop',
    stopsHelper: 'You can add stops to pick up passengers along the way',
    selectMultiple: 'Select Multiple',
    selectCities: 'Select Cities',
    dateLabel: 'Date',
    timeLabel: 'Time',
    seatsLabel: 'Number of Seats',
    seatsDescription: '1 to 8 seats',
    priceLabel: 'Price Per Seat',
    currencyLabel: 'Currency',
    // T-079 / T-080 — amenities, jo'natma, road pickup and the fill-up flag.
    amenitiesLabel: 'Amenities',
    amenityAirCon: 'Air conditioning',
    amenityWifi: 'Internet, WiFi',
    amenityRoofRack: 'Roof rack fitted',
    amenityTrailer: 'Trailer fitted',
    parcelAccepted: 'I carry parcels (post)',
    parcelPriceLabel: 'Parcel price (UZS)',
    parcelMaxKgLabel: 'Maximum (kg)',
    parcelMaxKgHelper: 'For example 20 — I carry parcels up to 20 kg.',
    roadPickupLabel: 'Only from the stand, or if you come out to the road',
    roadPickupPlaceholder: 'For example: from Quyliq market, or if you come out to the Fergana road',
    departsWhenFullLabel: 'I leave once the car fills up',
    departsWhenFullHelper: 'No fixed time — you set off as soon as the seats are taken.',
    // T-078 — the mockup's Narxlar list, payment and class.
    pricesSectionTitle: 'Prices',
    priceBackSalonLabel: 'Back salon price (optional)',
    priceBackSalonHelper: 'When one passenger books the whole back row',
    priceWholeSalonLabel: 'Whole salon price (optional)',
    priceWholeSalonHelper: 'When one passenger books the whole car',
    waitingFeeLabel: 'Waiting fee (UZS/min, optional)',
    waitingFeeHelper: 'Shown to the passenger. The app does not calculate or charge it.',
    freeWaitingLabel: 'Free waiting (min)',
    freeWaitingHelper: 'The waiting fee starts after this. 0 is allowed.',
    pickupFeeLabel: 'Door pickup fee (UZS)',
    pickupFeeHelper: 'Set 0 for free door pickup.',
    paymentLabel: 'Payment methods',
    paymentCash: 'Cash',
    paymentCard: 'Click, Payme',
    vehicleClassLabel: 'Vehicle class',
    vehicleClass_standard: 'Standard',
    vehicleClass_comfort: 'Comfort',
    vehicleClass_business: 'Business',
    vehicleClass_econom: 'Econom',
    vehicleClass_tourist: 'Tourist',

    noteLabel: 'Note (Optional)',
    notePlaceholder: 'e.g., My trunk is full, we will not stop on the way, we need to reach the airport in time',
    vehicleLabel: 'Vehicle',
    selectVehicle: 'Select Vehicle',
    noVehicle: 'No Vehicle Found',
    addVehicle: 'Add Vehicle',
    summary: 'Summary',
    route: 'Route',
    departureTime: 'Departure Time',
    totalSeats: 'Total Seats',
    pricePerSeat: 'Price Per Seat',
    createOffer: 'Create Offer',
    updateOffer: 'Update Offer',
    saveDraft: 'Save as Draft',
    creating: 'Creating...',
    updating: 'Updating...',
    createSuccess: 'Offer created successfully',
    updateSuccess: 'Offer updated successfully',
    errorMinAdvance: 'Departure time must be at least 30 minutes from now',
    errorInvalidSeats: 'Number of seats must be between 1 and 8',
    errorInvalidPrice: 'Price must be at least 5000',
    errorMissingFields: 'Please fill in all required fields',
  },

  // Offer Passengers
  offerPassengers: {
    title: 'Passenger Requests',
    all: 'All',
    pending: 'Pending',
    confirmed: 'Confirmed',
    rejected: 'Rejected',
    cancelled: 'Cancelled',
    confirm: 'Confirm',
    reject: 'Reject',
    confirmPassenger: 'Confirm Passenger',
    rejectPassenger: 'Reject Passenger',
    rejectReasonOptional: 'Provide a reason (optional):',
    rejectReasonPlaceholder: 'e.g., Offer is full',
    message: 'Message',
    rejectionReason: 'Rejection Reason',
    confirmSuccess: 'Passenger confirmed successfully',
    rejectSuccess: 'Passenger rejected',
    noPassengers: 'No passengers yet',
    noPassengersMessage: 'Passengers will appear here when they join your offer',
    noPassengersFiltered: 'No passengers in this status',
    noPassengersFilteredMessage: 'There are no passenger requests with this status',
    seatsRequested: '{count} seats requested',
    seatsRequestedOne: '1 seat requested',
    frontSeatRequested: 'Front seat requested',
    agreedPriceFrontSeat: 'Agreed: {price} {currency} (front seat)',
    agreedPriceMultiple: 'Agreed: {total} {currency} ({seats} seats, 1 front seat)',
    agreedPriceCalculation: 'Agreed: {price} {currency} × {seats} = {total} {currency}',
    unknownPassenger: 'Unknown',
    confirmPassengerMessage: 'Confirm {name} for {seats} seats?',
    confirmPassengerMessageOne: 'Confirm {name} for 1 seat?',
    contactTitle: 'Contact the passenger',
    noPhone: 'The passenger has no phone number on file',
  },
  /** T-101 step 15 — see uz.ts for the artboard the labels come from. */
  drawer: {
    open: "Menu",
    close: "Close menu",
    soon: "Soon",
    groupOffers: "Offers",
    offerCreate: "Create offer",
    offerMine: "My offers",
    groupOrders: "Orders",
    ordersIncoming: "Incoming orders",
    ordersMine: "My orders",
    cars: "My vehicles",
    messages: "Messages",
    balance: "Balance",
    income: "Income and expenses",
    profile: "My profile",
    groupDocs: "My documents",
    docsAll: "All documents",
    docsPassport: "Passport",
    docsLicense: "Driver's license",
    docsTechPassport: "Vehicle registration",
    docsTaxiLicense: "License",
    settings: "Settings",
    guides: "Guides",
  },

  // T-101 step 17 — the passenger-orders screen (DriverQidiruv). 17c: the card.
  // T-101 step 18 — the driver's rides screen (`DriverMyOrder`).
  myRides: {
    phaseJarayon: 'In progress',
    phaseFaol: 'Active rides',
    phaseTarix: 'Ride history',
    emptyJarayon: 'No rides in progress',
    emptyFaol: 'No active rides',
    emptyTarix: 'Your ride history is empty',
    stateCollecting: 'Filling up',
    stateConfirmed: 'Confirmed',
    stateDone: 'Completed',
    fillFull: 'Fully booked',
    fillOpen: 'Seats still open',
    incomeExpected: 'expected income',
    incomeEarned: 'income earned',
    statSeats: 'seats taken',
    statOrders: 'Orders',
    pastHint: 'The departure time has passed. Archive the ride to close it.',
    cancelRide: 'Cancel the ride',
    bookedAt: 'Booked at',
    bookingTitle: 'Booking details',
    passengersHeading: "Passengers on this ride",
    ordersCount: "{count} orders",
    reasonCarBroken: 'My car broke down.',
    reasonCarNotRunning: 'My car is not running.',
    reasonNoFuel: 'I could not refuel.',
    reasonPassengerLied: 'The passenger was not honest.',
    reasonNoAnswer: 'The passenger did not answer the phone.',
    reasonNotLeftHome: 'The passenger did not come out.',
    reasonNotAtAddress: 'The passenger was not at the agreed address.',
    reasonWrongAddress: 'The passenger chose the wrong address.',
    reasonOther: 'Other',
  },
  passengerOrders: {
    priceListed: "Listed price",
    pricePassenger: "Passenger's price",
    priceMine: "Your offer",
    kindOddiy: "Standard",
    kindMaxsus: "Special (paid)",
    people: "people",
    // 17d: the screen
    title: "Intercity",
    kindTaxi: "Taxi",
    kindParcel: "Parcel",
    modeIncoming: "Incoming orders",
    modeSent: "Send an offer",
    tabAll: "All",
    tabOddiy: "Standard",
    tabMaxsus: "Special order",
    sortMatch: "Best match",
    sortPrice: "Price",
    sortSeats: "Passengers",
    sortSoon: "Soonest",
    titleIncoming: "Orders from passengers",
    titleOddiy: "Standard orders — by listed price",
    titleMaxsus: "Special (paid) — by passenger's price",
    titleSent: "Sent offers",
    countOrders: "orders",
    countOffers: "offers",
    emptyIncoming: "No incoming orders",
    emptySent: "No sent offers",
    anyRoute: "All routes",
    // 17e: the sheet
    sheetIncoming: "Passenger's order",
    sheetSent: "Your offer",
    code: "Order",
    seatsTitle: "Requested seats",
    seatKind: "Seat type",
    specialTitle: "Special order prices",
    payment: "Payment",
    phone: "Phone",
    phoneHidden: "Shown once the passenger confirms",
    accept: "Accept",
    offer: "Send an offer",
    yourPrice: "Your price",
    thousands: "000",
    sendOffer: "Send the offer",
    notePlaceholder: "Note: pick-up on the way, large boot...",
    acceptConfirmTitle: "Accept this order?",
    acceptConfirmBody: "The passenger gets an offer at the listed price. The phone number opens once they confirm. Total:",
    // 17f: the result dialog
    resultAcceptTitle: "Order accepted",
    resultAcceptBody: "The passenger has been sent an offer at the listed price. Once they confirm, the phone number opens and you get a notification.",
    resultOfferBody: "The passenger will review the offer. You get a notification when they answer.",
    resultOfferPrice: "Offer price",
    resultTotal: "Total to pay",
    close: "Close",
  },
};
