/**
 * The app's active-offer count, and whether it still agrees with the SERVER's. T-115.
 *
...
// ── the rule ────────────────────────────────────────────────────────────────
ok('counts a published offer leaving later', M.isActiveOffer(offer('published', SOON), NOW) === true);
ok('does not count a cancelled offer', M.isActiveOffer(offer('cancelled', SOON), NOW) === false);
ok('does not count an archived offer', M.isActiveOffer(offer('archived', SOON), NOW) === false);
ok(
...
const apiStatuses = apiSrc.match(/DRIVER_ACTIVE_STATUSES\s*=\s*\[([^\]]*)\]/);
ok('the API still declares the driver status list', !!apiStatuses);
if (apiStatuses) {
  const parsed = apiStatuses[1]
    ...
    .filter(Boolean);
  ok(
    `the driver status list matches the API ([${parsed}] vs [${M.DRIVER_ACTIVE_STATUSES}])`,
    JSON.stringify(parsed) === JSON.stringify([...M.DRIVER_ACTIVE_STATUSES]),
  );
}

/*
 * 🔴 The artboard is the third copy of the number. `DriverMyOrder.dc.html` draws `MAX_ELON = 2`,
 * and it is what the owner asked for — if the code ever moves away from it, the design and the
 * product have diverged and someone should say so out loud.
 */
const board = path.resolve(root, '..', 'htmlDesign', 'DriverMyOrder.dc.html');
if (fs.existsSync(board)) {
  const drawn = fs.readFileSync(board, 'utf8').match(/MAX_ELON\s*=\s*(\d+)/);
  ok(
    `the artboard's MAX_ELON still matches (${drawn ? drawn[1] : '?'})`,
    !!drawn && Number(drawn[1]) === M.MAX_ACTIVE_OFFERS,
  );
}
...
console.log('✓ active offers: all ' + pass + ' assertions pass (rule · API parity · artboard)');
